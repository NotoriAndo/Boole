...
fn missing_valid_before_returns_400_bad_payload() {
    ...
    let event_path = dir.join("bounty-events.ndjson");
    let booted = boot(1, Some(mock_bounty_fixture_path()), event_path);
    let key = SigningKeyV2::from_dev_id("operator-missing-valid-before");
    let payload = json!({
        "schema": STATUS_SCHEMA,
        "id": "gamma-1",
        "newStatus": "withdrawn",
        "ts": 1800001700000_u64,
    });
    let envelope = signed_envelope(&payload, &key);
...
fn expired_valid_before_returns_401_envelope_expired() {
    ...
    let event_path = dir.join("bounty-events.ndjson");
    let booted = boot(1, Some(mock_bounty_fixture_path()), event_path);
    let key = SigningKeyV2::from_dev_id("operator-expired-valid-before");
    let mut payload = status_payload("gamma-1", "withdrawn", 1800001800000);
