...
#[derive(Debug, Subcommand)]
enum WalletCommand {
    /// Authenticate and copy an encrypted vault to a new 0600 backup file.
    Backup {
        #[arg(long)]
        vault: PathBuf,
        #[arg(long)]
    ...
    /// Recover an authenticated encrypted backup to a new vault; never overwrite.
    Restore {
        #[arg(long)]
        backup: PathBuf,
        #[arg(long)]
...
enum ProductCommand {
    ...
    /// Download and atomically install a signed product-v2 plus its embedded
    /// bootable guest-v2 payload. Product and guest trust roots are explicit
    /// and independent; transport URLs never grant authority.
    InstallBootable {
        #[arg(long = "base-url")]
        base_url: String,
        #[arg(long = "install-root")]
    ...
    match cli.command {
        ...
        Some(Command::Wallet { command }) => match command {
            WalletCommand::Backup {
                ...
                output,
                json,
            } => wallet_pubkey_envelope(
                &vault,
                json,
                "wallet.backup",
                "backup",
                &[std::ffi::OsStr::new("--output"), output.as_os_str()],
            ),
            WalletCommand::Restore {
                ...
                "wallet.restore",
                "restore",
                &[std::ffi::OsStr::new("--backup"), backup.as_os_str()],
            ),
        ...
        Some(Command::Product { command }) => match command {
            ...
            ProductCommand::InstallBootable {
                base_url,
                install_root,
                download_staging,
                product_trust_root_key_id,
...
/// Pin the installed sibling or an explicitly selected absolute agent path.
/// Unlike ordinary command discovery, wallet secrets never use PATH fallback.
fn resolve_wallet_agent_binary() -> anyhow::Result<PathBuf> {
    boole_miner::resolve_wallet_agent_binary().map_err(anyhow::Error::msg)
}
...
fn spawn_wallet_agent(args: &[&std::ffi::OsStr]) -> anyhow::Result<String> {
    let agent_bin = resolve_wallet_agent_binary()?;
    let response = if args
        .first()
        .is_some_and(|arg| *arg == std::ffi::OsStr::new("sign"))
    {
        boole_miner::WalletAgentResponse::Signature
    } else {
        boole_miner::WalletAgentResponse::PublicKey
    };
    boole_miner::run_wallet_agent(
        &agent_bin,
        args,
        boole_miner::WalletAgentInput::Inherit,
        response,
    )
...
fn wallet_pubkey_envelope(
    ...
    let stdout = spawn_wallet_agent(&args)?;
    let trimmed = &stdout;
    if json {
...
fn product_install_boot_contract(
    ...
    let default_staging = download_staging.is_none().then(|| {
        let nonce = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        std::env::temp_dir().join(format!(
            "boole-curl-bootable-download-{}-{nonce}",
            std::process::id()
        ))
    });
    let requested_staging = download_staging.unwrap_or_else(|| {
    ...
    .unwrap_or_else(|error| {
        let reason = match &error {
            ...
            CurlProductTransportError::Verify(_) => "release-rejected",
            CurlProductTransportError::Install(boole_core::CurlProductInstallError::Verify(_))
            | CurlProductTransportError::Install(
                boole_core::CurlProductInstallError::GuestVerify(_),
            ) => "release-rejected",
            CurlProductTransportError::Install(_) => "install-rejected",
            CurlProductTransportError::Io(_) => "staging-io-failed",
        };
