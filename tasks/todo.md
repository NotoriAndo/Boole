...
- [x] **운영자 반증·판정 전환 (2026-07-20)**: 초판 §3-7이 p_surv·p_solve를
      ...
      재설계.
- [ ] **재설계 순서 (운영자 지시)**: ① 고정 epoch 발행 경제 재설계 페이퍼
      (본드=처벌 전용 / 발행=유용 출제·해결 구매 / 총발행 고정 / 난이도
      보상은 목표 성공률 밴드 피크 / 항상 reveal / 분모=공개 전 티켓 배정자
      고정 / 창 닫힌 후 보상 확정) — **발행-원천은 자기거래 채널을 재개방
      하므로 시뮬레이션 게이트는 "정직 EV>0 ∧ 전 담합 EV<0" 동시 만족** ②
      담합·Sybil·보류 에이전트 시뮬레이션 ③ 통과 후에만 최소 ZK 라이브러리
      (유한체+간단 회로만 — 전체 선저작 금지) + 소형 LLM 스파이크. 착수는
      운영자 승인 대기. 전 과정 코드·합의·checker pin·기존 Base 무변경.
