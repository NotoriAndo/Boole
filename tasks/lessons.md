...
3. **결정적·오픈소스 생성기는 planted 난이도를 생성기-전지식 공격자에게
   못 숨긴다.** 채굴 문제를 seed에서 결정적으로 생성하면 채굴자도 같은
   생성기를 돌려 심어둔 구조(자유·변이 위치·역산 경로)를 안다. "생성기를
   다 알아도 여전히 어려운가"가 성립 조건 1번이고, 이건 설계로 장담 못 하고
   **실측**해야 한다(ZK.0 하네스 = 그 실측틀).
