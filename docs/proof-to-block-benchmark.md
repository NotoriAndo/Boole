...
For model-to-model comparison, prefer `--isolated-node-per-row` on `preflight-model-benchmark.sh` or `phase7-solo-preflight.sh`. That wrapper runs each generated model row through `scripts/isolated-node-model-row.sh`, giving the row its own fresh `boole-node run-local`, block store, reward store, TCP port, and quota state. This avoids shared-node contamination where an earlier model advances the head or consumes IP quota before later rows. Use `--isolated-node-base-port PORT` to choose the first port; each row increments by one. Do not combine shared-node sequential evidence with model-ranking claims.

For closed v1 mining-productivity runs, use `fixtures/benchmarks/closed-mining-productivity-v1/scenario.json` instead of the `boole-node run-local` default `fixtures/protocol/runtime-smoke/v1.json`. This scenario is explicitly for `boole-miner start --profile v1-lenbound` / `FamilyV1LengthBoundTargetEmitter` runs, not for `scripts/boole-model-benchmark.py`'s default `--benchmark-mode mining` / `boole.calibration.pow.v1` rfl-style benchmark path. The closed-productivity scenario copies the calibrated v1 difficulty thresholds from `fixtures/protocol/config/v1.json#cases[name=valid]` (`T_submit`, `T_share`, `T_block`, `T_ticket`) and keeps the problem shape fields (`L`, `D_max`, `EMAWindow`, `M`) aligned, but lifts productivity-contaminating caps to `100000`: `K_max`, `ShareCapPerPK_Block`, `SharePoolGlobalCap`, and `perIpRateLimitPer60s`. `K_max` remains the block selected-share cap, while `SharePoolGlobalCap` is the node admission cap; they are intentionally separate so long model runs are not rejected by `GlobalCapExceeded` before measuring proof/share/block productivity. This scenario is for closed local productivity evidence only; it is not public-network mining and it is not an admission/rate-limit policy test.
