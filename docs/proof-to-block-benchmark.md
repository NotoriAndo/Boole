...
For closed v1 mining-productivity runs, use `fixtures/benchmarks/closed-mining-productivity-v1/scenario.json` instead of the `boole-node run-local` default `fixtures/protocol/runtime-smoke/v1.json`. This scenario is explicitly for `boole-miner start --profile v1-lenbound` / `FamilyV1LengthBoundTargetEmitter` runs, not for `scripts/boole-model-benchmark.py`'s default `--benchmark-mode mining` / `boole.calibration.pow.v1` rfl-style benchmark path. The closed-productivity scenario copies the calibrated v1 difficulty fields from `fixtures/protocol/config/v1.json#cases[name=valid]` (`T_submit`, `T_share`, `T_block`, `T_ticket`, `K_max`, `L`, `D_max`, `EMAWindow`, `M`) but raises `ShareCapPerPK_Block` and `perIpRateLimitPer60s` to `100000` so anti-spam limits do not dominate model productivity measurements. This scenario is for closed local productivity evidence only; it is not public-network mining and it is not an admission/rate-limit policy test.
