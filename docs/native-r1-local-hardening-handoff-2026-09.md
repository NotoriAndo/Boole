...
| Normal shutdown while all 8/4/8 inputs remain held at 131,073 balances | 5.071s stop; all eight HTTP/twelve TLS connections closed without completing missing input; 51.803s replay while old clients/runtime remain alive; 610.375MiB RSS; exact original state | [Separate held-input criteria/result](native-http-shutdown-drain-2026-09.md#large-held-input-result--first-run-pass); no in-flight ledger mutation in this scenario, not a whole-process CPU/disk time guarantee |
