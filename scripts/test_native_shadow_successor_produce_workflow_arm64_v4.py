#!/usr/bin/env python3
"""Contract tests for the production-complete launcher-v2 v4 workflow."""
...
WORKFLOW = REPO / ".github/workflows/native-shadow-successor-produce-arm64-v4.yml"
...
class WorkflowTopologyTests(unittest.TestCase):
    def test_manual_workflow_has_the_four_exact_jobs_and_two_modes(self) -> None:
        ...
        for automatic in ("pull_request:", "push:", "schedule:", "workflow_run:"):
    ...
    def test_qualified_replicas_are_kept_seven_days_and_compared_byte_exact(self) -> None:
        ...
        self.assertIn(
            "native-shadow-successor-v4-replica-${{ matrix.replica }}", produce
        )
        upload = produce[produce.index("upload-artifact@") :]
        self.assertIn("path: |", upload)
        self.assertIn(
            "${{ runner.temp }}/successor-v4-replica-${{ matrix.replica }}/outputs",
            upload,
        )
        self.assertIn(
            "${{ runner.temp }}/successor-v4-replica-${{ matrix.replica }}/REPLICA-PROVENANCE.json",
            upload,
        )
        self.assertIn("needs: [production-authority-guard, produce]", compare)
        ...
        for replica in ("1", "2"):
            self.assertIn(f"native-shadow-successor-v4-replica-{replica}", compare)
    ...
    def test_rehearsal_collection_has_exactly_one_plain_single_link_member(self) -> None:
        body = workflow_job("free-rehearsal")
        collection = body[body.index("Require exactly one canonical R2 JSON member") :]
        self.assertIn(
...
class RootExecutionAnchorContractTests(unittest.TestCase):
    def test_rehearsal_completes_history_without_importing_tags_for_bundle_fsck(self) -> None:
        ...
        checkout = rehearsal[
            rehearsal.index("actions/checkout@") : rehearsal.index(
                "Verify the complete preregistered v4 bindings first"
            )
        ]
    ...
    def test_rehearsal_root_edge_executes_only_one_verified_head_anchor(self) -> None:
        ...
        self.assertNotIn(
            "sudo ./scripts/native-shadow-successor-produce-arm64-v4.sh",
            rehearsal,
        )
    ...
    def test_root_edge_never_executes_checkout_shell_or_python(self) -> None:
        ...
        for forbidden in (
            "sudo ./scripts/native-shadow-successor-produce-arm64-v4.sh",
            'parent="$RUNNER_TEMP/successor-v4-replica-',
            'sudo /usr/bin/python3 -I -S "$GITHUB_WORKSPACE/',
            'sudo python3 ',
        ):
            self.assertNotIn(forbidden, produce)
        self.assertIn(
            "readonly base=/var/lib/boole/native-shadow-successor-v4", produce
        )
    ...
    def test_root_wrapper_refuses_every_non_anchor_execution_root(self) -> None:
        ...
        self.assertIn(
            "/var/lib/boole/native-shadow-successor-v4/anchors/", validator
        )
    ...
    def test_production_export_has_only_root_owned_nonwritable_ancestry(self) -> None:
        ...
        self.assertIn(
            "/var/lib/boole/native-shadow-successor-v4/exports/", validator
        )
...
class WrapperSurfaceTests(unittest.TestCase):
    ...
    def test_dispatch_claim_ignores_foreign_cwd_and_injected_git_dir(self) -> None:
        ...
        if a6_in_head:
            ...
            self.assertIn("the HEAD A6 blob cannot be hashed", completed.stderr)
    ...
    def test_dispatch_claim_adapters_finish_before_host_or_output_work(self) -> None:
        ...
        self.assertIn('--workflow-path "$v4_workflow_path"', source)
        self.assertIn('--head-sha "$head_sha"', source)
        self.assertIn('--head-a6-sha256 "$head_a6_sha256"', source)
    ...
    def test_dispatch_claim_uses_live_git_facts_without_normalising_them(self) -> None:
        ...
        self.assertIn('git_repo cat-file blob "$head_sha:$a6_relative"', source)
        ...
        self.assertIn(
            'workflow_prefix="$github_repository/$v4_workflow_path@"', source
        )
        self.assertIn('[[ $github_workflow_ref == "$workflow_prefix"* ]]', source)
    ...
    def test_dispatch_attempt_id_is_parsed_from_the_same_head_blob(self) -> None:
        ...
        head_blob = 'git_repo cat-file blob "$head_sha:$a6_relative"'
        self.assertEqual(block.count(head_blob), 2)
        self.assertIn("sys.stdin.buffer.read(1048577)", block)
        self.assertNotIn('"$ROOT/$a6_relative"', block)
    ...
    def test_binding_only_mode_rejects_missing_or_tampered_p3_before_import(self) -> None:
        ...
        p2 = json.loads((REPO / p2_relative).read_text(encoding="utf-8"))
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            wrapper = root / "scripts/native-shadow-successor-produce-arm64-v4.sh"
            ...
            for relative in (
                p2_relative,
                p3_relative,
                *(row["path"] for row in p2["bindings"]),
            ):
            ...
            self.assertIn("P3", missing.stderr)

            p3.write_bytes((REPO / p3_relative).read_bytes() + b"\n")
            ...
            self.assertIn("P3", tampered.stderr)
    ...
    def test_authority_only_mode_refuses_before_host_or_output_work(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertNotEqual(completed.returncode, 0)
            self.assertIn("required binding is absent", completed.stderr.lower())
    ...
    def test_wrapper_and_workflow_never_call_a_historical_generation(self) -> None:
        ...
        self.assertIn("scripts/native-shadow-successor-produce-arm64-v4.sh", combined)
        core = (REPO / "scripts/native_shadow_successor_produce_phase_arm64_v4.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_production_rechecks_authority_then_preflights_and_calls_core_once(self) -> None:
        ...
        self.assertIn('--pending "$outputs/PRODUCE-RESULT-PENDING-READBACK-V4.json"', source)
    ...
    def test_production_requires_root_and_the_future_workflow_uses_one_sudo_edge(self) -> None:
        ...
        self.assertNotIn(
            "sudo ./scripts/native-shadow-successor-produce-arm64-v4.sh", produce
        )
        self.assertIn('"$anchored_wrapper" --production', produce)
        rehearsal = workflow_job("free-rehearsal")
        self.assertNotIn("sudo ./scripts/native-shadow-successor-produce-arm64-v4.sh", rehearsal)
    ...
    def test_collectable_parent_contains_exactly_the_one_production_output(self) -> None:
        ...
        producer = (
            REPO / "scripts/native_shadow_successor_produce_phase_arm64_v4.py"
        ).read_text(encoding="utf-8")
        self.assertIn(
    ...
    def test_production_recovery_root_has_a_real_root_owned_nonwritable_ancestry(
        ...
        for path in (
            "/run",
            "/run/boole",
            "/run/boole/native-shadow-successor-v4",
        ):
        ...
        self.assertNotIn(
            "mkdir -p -m 0700 -- /run/boole/native-shadow-successor-v4",
            production,
        )
    ...
    def test_linux_crashed_wrapper_cleanup_removes_all_claim_units(self) -> None:
        ...
        stem = f"boole-nsv4-lifecycle-{unique}"
        ...
        with tempfile.TemporaryDirectory(prefix="boole-nsv4-lifecycle-") as directory:
            root = pathlib.Path(directory)
            ready = root / "READY"
    ...
    def test_transient_unit_gc_races_converge_only_after_absence_is_proved(
        ...
        harness = (
            ...
            f"{functions}\n"
            "stop_and_verify_unit boole-nsv4-test-race.service\n"
        )
