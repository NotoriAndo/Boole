...
install -m 0644 \
  ...
  "$stage/usr/lib/systemd/system/boole-native-shadow-replay-node.service"
install -m 0644 \
...
install -m 0755 /bin/true \
  "$stage/usr/libexec/boole/boole-native-shadow-replay-node"
...
replay_node_metadata=$(stat -c %U:%G:%a \
  "$stage/usr/libexec/boole/boole-native-shadow-replay-node")
[[ "$replay_node_metadata" == root:root:755 ]] \
  || die "staged replay node is not root:root mode 0755"
...
state_parent_metadata=$(stat -c %u:%g:%a "$stage/var/lib/boole/native-shadow")
[[ "$state_parent_metadata" == 0:0:755 ]] \
  || die "native-shadow state parent differs from root:root mode 0755"
state_mode=$(stat -c %a "$stage/var/lib/boole/native-shadow/node-state")
state_uid=$(stat -c %u "$stage/var/lib/boole/native-shadow/node-state")
state_gid=$(stat -c %g "$stage/var/lib/boole/native-shadow/node-state")
[[ "$state_mode" == 700 ]] \
  || die "native-shadow node-state directory mode differs from 0700"
[[ "$state_uid" -eq "$node_uid" && "$state_gid" -eq "$node_gid" ]] \
  || die "native-shadow node-state ownership differs from boole-node:boole-node"
