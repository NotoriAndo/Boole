...
class OfficialMirrorSeedTests(unittest.TestCase):
    ...
    def test_snapshot_pool_paths_map_only_to_the_official_architecture_mirror(self):
        ...
        for forbidden in (
            ...
            "https://snapshot.ubuntu.com/ubuntu/20240425T160000Z/dists/noble/InRelease",
        ):
    ...
    def test_tracked_boot_and_runtime_sets_are_covered_without_metadata(self):
        ...
        self.assertEqual(len(boot), 193)
        self.assertTrue(all("/pool/" in row["url"] for row in boot))
        self.assertEqual(len({row["sha256"] for row in boot}), 193)
