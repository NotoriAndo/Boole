...
P4 -> R3 -> F7 -> A7 chain has been checked.  Repository build modules are
...
P4_STATUS = "A6-WITHHELD-PENDING-MAIN-ONLY-SUCCESSOR-GENERATION"
P4_FIELD = "mainBranchDispatchFenceCorrection"
ATTEMPT_ID_PATTERN = r"^[a-z0-9][a-z0-9._-]{0,127}$"
...
A7_PATH = "native/containment/native-shadow-mac3-successor-production-authority-arm64-v7.json"
RESULT_V7_PATH = "native/containment/native-shadow-mac3-successor-image-production-result-arm64-v7.json"
RESULT_V7_SCHEMA = (
...
R3_SCHEMA = (
    "boole.native-shadow.mac3.launcher-v2-successor-producer-rehearsal.arm64.v3"
)
R3_STATUS = "PASS-NO-IMAGE-PRODUCED"
...
F7_SCHEMA = "boole.native-shadow.mac3.successor-producer-fingerprint.arm64.v7"
F7_STATUS = "SEALED-AFTER-FRESH-R3-PRODUCTION-GENERATION-NOT-AUTHORISED"
A7_SCHEMA = "boole.native-shadow.mac3.successor-production-authority.arm64.v7"
A7_STATUS = "ONE-NAMED-PRODUCTION-RUN-AUTHORISED-NOT-RUN"
...
PENDING_RESULT_NAME = "PRODUCE-RESULT-PENDING-READBACK-V5.json"
...
REPLICA_ARTIFACT_PREFIX = "native-shadow-successor-v5-replica-"
...
class SuccessorProduceV5Error(RuntimeError):
...
def _module_name_from_relative(relative: str) -> str:
    ...
    if pure.suffix != ".py":
        raise SuccessorProduceV5Error(f"bound repository module is not Python: {relative}")
    parts = list(pure.with_suffix("").parts)
    if parts[-1] == "__init__":
        parts.pop()
    if not parts or any(not part.isidentifier() for part in parts):
        raise SuccessorProduceV5Error(f"bound repository module path is invalid: {relative}")
    return ".".join(parts)
    ...
    def read_metadata(
        ...
        *,
        ...
        try:
            ...
            raise SuccessorProduceV5Error(
                f"{context} was not pinned before validation"
            ) from exc
        if expected.size_bytes > max_bytes:
            raise SuccessorProduceV5Error(f"metadata exceeds byte limit: {name}")
        before = os.fstat(descriptor)
        if self._stat_key(before) != self._identity_key(expected):
            raise SuccessorProduceV5Error(f"{context} changed while it was pinned")
        ...
        try:
            ...
            raise SuccessorProduceV5Error(f"{context} is unreadable") from exc
        raw = b"".join(chunks)
...
def _safe_path(root: pathlib.Path, relative: str) -> pathlib.Path:
    ...
    if not relative or pure.is_absolute() or ".." in pure.parts or pure.as_posix() != relative:
        raise SuccessorProduceV5Error(f"unsafe repository binding path: {relative!r}")
    ...
    if not contained:
        raise SuccessorProduceV5Error(f"repository binding leaves root: {relative}")
    return candidate
...
def _read_regular(
    ...
    *,
    ...
    except SuccessorProduceV5Error:
        raise
    except OSError as exc:
        raise SuccessorProduceV5Error(f"required binding is unreadable: {relative}") from exc
    finally:
...
def _descriptor_output_identity(descriptor: int, context: str) -> PinnedOutputIdentity:
    ...
    if not stat.S_ISREG(before.st_mode):
        raise SuccessorProduceV5Error(f"{context} is not a regular file")
    ...
    try:
        for name in names:
            if pathlib.PurePosixPath(name).name != name:
                raise SuccessorProduceV5Error(f"unsafe output name: {name}")
            try:
                descriptor = os.open(
...
def _load_canonical(root: pathlib.Path, relative: str) -> tuple[FileIdentity, dict[str, Any]]:
    ...
    try:
        ...
        raise SuccessorProduceV5Error(f"binding is not canonical JSON: {relative}") from exc
    if not isinstance(document, dict) or raw != canonical_json(document):
        raise SuccessorProduceV5Error(f"binding is not one canonical JSON object: {relative}")
    return identity, document


def _require_identity(actual: FileIdentity, expected: Mapping[str, Any], context: str) -> None:
    if actual.record() != dict(expected):
        raise SuccessorProduceV5Error(f"{context} identity differs")


def _require_exact_keys(document: Mapping[str, Any], expected: set[str], context: str) -> None:
    if set(document) != expected:
        raise SuccessorProduceV5Error(f"{context} keys differ")
...
def _require_strict_equal(actual: Any, expected: Any, context: str) -> None:
    if not _strictly_equal(actual, expected):
        raise SuccessorProduceV5Error(f"{context} differs")


def _require_absent(root: pathlib.Path, relative: str, context: str) -> None:
    path = _safe_path(root, relative)
    if os.path.lexists(path):
        raise SuccessorProduceV5Error(f"{context} must remain absent: {relative}")
...
def dispatch_claim_ref(attempt_id: str) -> str:
    ...
    if not isinstance(attempt_id, str) or re.fullmatch(
        ATTEMPT_ID_PATTERN, attempt_id
    ) is None:
        raise SuccessorProduceV5Error(
            "dispatch claim attempt ID violates the P4 pattern"
        )
...
def _dispatch_runtime_context(
    ...
    *,
    ...
    if not isinstance(github_run_id, str) or re.fullmatch(
        ...
        raise SuccessorProduceV5Error("dispatch claim GitHub run ID differs")
    if type(github_run_attempt) is not str or github_run_attempt != "1":
        raise SuccessorProduceV5Error("dispatch claim GitHub run attempt differs")
    if type(event_name) is not str or event_name != EXACT_EVENT_NAME:
        raise SuccessorProduceV5Error("dispatch claim event name differs")
    if type(dispatch_ref) is not str or dispatch_ref != EXACT_DISPATCH_REF:
        raise SuccessorProduceV5Error("dispatch claim dispatch ref differs")
    if type(workflow_path) is not str or workflow_path != V5_WORKFLOW_PATH:
        raise SuccessorProduceV5Error("dispatch claim workflow path differs")
    if type(workflow_ref) is not str or workflow_ref != EXACT_WORKFLOW_REF:
        raise SuccessorProduceV5Error("dispatch claim workflow ref differs")
    if not isinstance(head_sha, str) or re.fullmatch(
        r"[0-9a-f]{40}", head_sha
    ) is None:
        raise SuccessorProduceV5Error("dispatch claim head SHA differs")
    if not isinstance(head_authority_sha256, str) or re.fullmatch(
        r"[0-9a-f]{64}", head_authority_sha256
    ) is None:
        raise SuccessorProduceV5Error("dispatch claim HEAD A7 digest differs")
    if head_authority_sha256 != chain.identities["A7"].sha256:
        raise SuccessorProduceV5Error("dispatch claim HEAD A7 digest differs")
...
def dispatch_claim_document(
    ...
    *,
    ...
    if not isinstance(chain, GenerationChain):
        raise SuccessorProduceV5Error("dispatch claim has no verified A7 chain")
    _dispatch_runtime_context(
        ...
        head_sha=head_sha,
        head_authority_sha256=head_authority_sha256,
    )
    dispatch_claim_ref(chain.attempt_id)
    return {
        "authoritySha256": chain.identities["A7"].sha256,
...
def verify_dispatch_claim(
    ...
    *,
    ...
    """Recheck one repository claim against live A7 and workflow context."""

    expected_ref = dispatch_claim_ref(chain.attempt_id)
    if claim_ref != expected_ref:
        raise SuccessorProduceV5Error("dispatch claim ref differs")
    ...
    if not isinstance(raw_message, bytes) or raw_message != expected_raw:
        raise SuccessorProduceV5Error("dispatch claim message differs")
    return expected
...
def _git_dispatch_read(
    ...
    *,
    ...
    if supplied_root.is_symlink() or not stat.S_ISDIR(supplied_info.st_mode):
        raise SuccessorProduceV5Error("dispatch claim live repository differs")
    try:
    ...
    if max_bytes < 1 or max_bytes > MAX_METADATA_BYTES + 1:
        raise SuccessorProduceV5Error("dispatch claim Git read bound differs")
    command = (
        "/usr/bin/git",
        "--no-replace-objects",
        "-c",
...
def verify_dispatch_repository_context(
    repository_root: pathlib.Path,
    chain: GenerationChain,
    *,
    github_run_id: str,
    ...
    """Recheck the main-only dispatch and live HEAD/A7 before any effect."""
    ...
    try:
        ...
        raise SuccessorProduceV5Error(
            "dispatch context live repository is unavailable"
        ) from exc
    if supplied_root.is_symlink() or not stat.S_ISDIR(root_info.st_mode):
        raise SuccessorProduceV5Error("dispatch context live repository differs")
    observed_top = _git_dispatch_scalar(
        root, ("rev-parse", "--show-toplevel"), "context root"
    )
    try:
        top = pathlib.Path(observed_top).resolve(strict=True)
    except (OSError, RuntimeError) as exc:
        raise SuccessorProduceV5Error(
            "dispatch context live repository root differs"
        ) from exc
    if top != root:
        raise SuccessorProduceV5Error("dispatch context live repository root differs")
    observed_head = _git_dispatch_scalar(
        root, ("rev-parse", "--verify", "HEAD^{commit}"), "context HEAD"
    )
    if observed_head != head_sha:
        raise SuccessorProduceV5Error("dispatch context live HEAD differs")
    authority_object = f"{head_sha}:{A7_PATH}"
    raw_size = _git_dispatch_scalar(
        root, ("cat-file", "-s", authority_object), "context A7 size"
    )
    try:
        size = int(raw_size, 10)
    except ValueError as exc:
        raise SuccessorProduceV5Error("dispatch context live A7 size differs") from exc
    if size != chain.identities["A7"].size_bytes or size > MAX_METADATA_BYTES:
        raise SuccessorProduceV5Error("dispatch context live A7 size differs")
    raw_authority = _git_dispatch_read(
        root, ("cat-file", "blob", authority_object), max_bytes=size
    )
    if len(raw_authority) != size or _sha256(raw_authority) != head_authority_sha256:
        raise SuccessorProduceV5Error("dispatch context live A7 differs")
    final_head = _git_dispatch_scalar(
        root, ("rev-parse", "--verify", "HEAD^{commit}"), "final context HEAD"
    )
    if final_head != head_sha:
        raise SuccessorProduceV5Error("dispatch context live repository changed")
...
def _verify_live_dispatch_repository(
    *,
    ...
    if observed_top_level != root:
        raise SuccessorProduceV5Error("dispatch claim live repository root differs")

    observed_head = _git_dispatch_scalar(
        root, ("rev-parse", "--verify", "HEAD^{commit}"), "HEAD"
    )
    if observed_head != head_sha:
        raise SuccessorProduceV5Error("dispatch claim live HEAD differs")
    a7_object = f"{head_sha}:{A7_PATH}"
    raw_a7_size = _git_dispatch_scalar(
        root, ("cat-file", "-s", a7_object), "A7 size"
    )
    try:
        a7_size = int(raw_a7_size, 10)
    except ValueError as exc:
        raise SuccessorProduceV5Error(
            "dispatch claim live A7 size differs"
        ) from exc
    if a7_size != chain.identities["A7"].size_bytes or a7_size > MAX_METADATA_BYTES:
        raise SuccessorProduceV5Error("dispatch claim live A7 size differs")
    raw_a7 = _git_dispatch_read(
        root,
        ("cat-file", "blob", a7_object),
        max_bytes=a7_size,
    )
    if (
        len(raw_a7) != a7_size
        or _sha256(raw_a7) != chain.identities["A7"].sha256
    ):
        raise SuccessorProduceV5Error("dispatch claim live A7 differs")
    ...
    if observed_ref != ref_object_sha or observed_tag != tag_object_sha:
        raise SuccessorProduceV5Error("dispatch claim live ref object differs")
    if _git_dispatch_scalar(
        root, ("cat-file", "-t", tag_object_sha), "object type"
    ) != "tag":
        raise SuccessorProduceV5Error("dispatch claim live object type differs")
    ...
    if tag_size != len(raw_tag_object) or tag_size > 16_384:
        raise SuccessorProduceV5Error("dispatch claim live tag size differs")
    ...
    if observed_raw_tag != raw_tag_object:
        raise SuccessorProduceV5Error("dispatch claim live tag bytes differ")
    if _git_dispatch_scalar(
        root, ("rev-parse", "--verify", f"{claim_ref}^{{commit}}"), "target"
    ) != head_sha:
        raise SuccessorProduceV5Error("dispatch claim live target differs")
    ...
    if final_head != head_sha or final_ref != tag_object_sha:
        raise SuccessorProduceV5Error("dispatch claim live repository changed")
    ...
    *,
    ...
    for label, value in (
        ...
        if not isinstance(value, str) or re.fullmatch(r"[0-9a-f]{40}", value) is None:
            raise SuccessorProduceV5Error(f"dispatch claim {label} SHA differs")
    if ref_object_sha != tag_object_sha:
        raise SuccessorProduceV5Error("dispatch claim ref object differs")
    if not isinstance(raw_tag_object, bytes) or not raw_tag_object:
        raise SuccessorProduceV5Error("dispatch claim annotated tag differs")
    ...
    # live A7 SHA-256 and target commit checks; this reproduces Git's object ID.
    ...
    if computed_sha != tag_object_sha:
        raise SuccessorProduceV5Error("dispatch claim tag object SHA differs")
...
def _verified_dispatch_capability(
    ...
    if capability is None:
        raise SuccessorProduceV5Error("production dispatch capability is required")
    expected_keys = {
        "claim_ref",
        "ref_object_sha",
        "tag_object_sha",
    ...
    if not isinstance(capability, Mapping) or set(capability) != expected_keys:
        raise SuccessorProduceV5Error("production dispatch capability shape differs")
    raw_tag_object = capability["raw_tag_object"]
    if type(raw_tag_object) is not bytes:
        raise SuccessorProduceV5Error("production dispatch capability bytes differ")
    try:
        frozen = DispatchCapability(
            ...
            event_name=capability["event_name"],
            dispatch_ref=capability["dispatch_ref"],
            workflow_path=capability["workflow_path"],
            workflow_ref=capability["workflow_ref"],
            head_sha=capability["head_sha"],
            head_authority_sha256=capability["head_authority_sha256"],
        )
...
def _reverify_dispatch_capability(
    ...
    if not isinstance(capability, DispatchCapability):
        raise SuccessorProduceV5Error("production dispatch capability differs")
...
def verify_preregistered_generation(
    ...
    """Verify pinned P4 and the exact fresh producer-generation v5 files."""

    root = pathlib.Path(repository_root).resolve()
    try:
        p4_id, p4 = _load_canonical(root, P4_PATH)
    except SuccessorProduceV5Error as exc:
        raise SuccessorProduceV5Error(f"P4 correction is invalid: {exc}") from exc
    if p4_id.sha256 != P4_SHA256 or p4_id.size_bytes != P4_SIZE_BYTES:
        raise SuccessorProduceV5Error("P4 pinned identity differs")
    if p4.get("schema") != P4_SCHEMA or p4.get("status") != P4_STATUS:
        raise SuccessorProduceV5Error("P4 schema or status differs")
    if p4.get("generationLabel") != "P4":
        raise SuccessorProduceV5Error("P4 generation label differs")
    _require_strict_equal(p4.get("authorisations"), ZERO_AUTHORISATIONS, "P4 authority")
    _require_strict_equal(p4.get("runs"), P4_ZERO_RUNS, "P4 run accounting")
    ...
    correction = p4.get("correction")
    if not isinstance(correction, Mapping):
        raise SuccessorProduceV5Error("P4 dispatch correction differs")
    if correction.get("exactEventName") != EXACT_EVENT_NAME:
        raise SuccessorProduceV5Error("P4 event name differs")
    if correction.get("exactDispatchRef") != EXACT_DISPATCH_REF:
        raise SuccessorProduceV5Error("P4 dispatch ref differs")
    if correction.get("exactWorkflowRef") != EXACT_WORKFLOW_REF:
        raise SuccessorProduceV5Error("P4 workflow ref differs")
    future_binding = p4.get("futureBindingRequirement")
    _require_strict_equal(
        future_binding,
        {
            "correctionPath": P4_PATH,
        ...
        "P4 future binding requirement",
    )
    successor = p4.get("successorGeneration")
    if not isinstance(successor, Mapping):
        raise SuccessorProduceV5Error("P4 successor generation differs")
    if successor.get("futureFiles") != list(V5_PATHS):
        raise SuccessorProduceV5Error("P4 does not declare the exact five v5 paths")
    _require_strict_equal(
        successor.get("futureRecords"),
        {
            "authority": A7_PATH,
            "fingerprint": F7_PATH,
            "productionResult": RESULT_V7_PATH,
            "rehearsalResult": R3_PATH,
        },
        "P4 future records",
    )
    if successor.get("implementedByThisRecord") is not False or successor.get(
        "requiresFreshAuthorityZeroRehearsal"
    ) is not True:
        raise SuccessorProduceV5Error("P4 successor boundary differs")
    namespace = p4.get("successorClaimNamespace")
    if not isinstance(namespace, Mapping):
        raise SuccessorProduceV5Error("P4 successor namespace differs")
    _require_strict_equal(
        namespace.get("authority"),
        {
    ...
    _require_strict_equal(
        namespace.get("authorityContextFields"),
        ["dispatchRef", "eventName", "workflowPath", "workflowRef"],
        "P4 authority context fields",
    )
    claim_namespace = namespace.get("claim")
    if not isinstance(claim_namespace, Mapping):
        raise SuccessorProduceV5Error("P4 claim namespace differs")
    _require_strict_equal(
        claim_namespace.get("messageFields"),
        [
            "authoritySha256",
            "attemptId",
            "dispatchRef",
        ...
        "P4 canonical claim fields",
    )
    if claim_namespace.get("schema") != DISPATCH_CLAIM_SCHEMA or claim_namespace.get(
        "refTemplate"
    ) != f"{DISPATCH_CLAIM_REF_PREFIX}{{attemptId}}":
        raise SuccessorProduceV5Error("P4 claim namespace differs")
    _require_strict_equal(
    ...
    reservations = p4.get("unusedReservations")
    if not isinstance(reservations, list) or len(reservations) != 2:
        raise SuccessorProduceV5Error("P4 unused reservation set differs")
    for index, row in enumerate(reservations):
        if not isinstance(row, Mapping) or set(row) != {
            "authorityEverGranted",
            "path",
            "requiredAbsent",
            "reuseForbidden",
        }:
            raise SuccessorProduceV5Error("P4 unused reservation differs")
        if row.get("authorityEverGranted") is not False or row.get(
            "requiredAbsent"
        ) is not True or row.get("reuseForbidden") is not True:
            raise SuccessorProduceV5Error("P4 unused reservation differs")
        path = row.get("path")
        if not isinstance(path, str):
            raise SuccessorProduceV5Error("P4 unused reservation path differs")
        _require_absent(root, path, f"P4 unused reservation {index + 1}")
    historical = p4.get("historicalGeneration", {}).get("files")
    if not isinstance(historical, list) or len(historical) != 5:
        raise SuccessorProduceV5Error("P4 historical generation differs")
    for row in historical:
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise SuccessorProduceV5Error("P4 historical file binding differs")
        identity = _read_regular(root, row["path"])[0]
        _require_identity(identity, row, "P4 historical file")
    predecessors = p4.get("predecessors")
    if not isinstance(predecessors, list) or len(predecessors) != 6:
        raise SuccessorProduceV5Error("P4 predecessor set differs")
    for row in predecessors:
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise SuccessorProduceV5Error("P4 predecessor binding differs")
        identity = _read_regular(root, row["path"])[0]
        _require_identity(identity, row, "P4 predecessor")

    generation_identities = tuple(_read_regular(root, path)[0] for path in V5_PATHS)
    reused_identities = tuple(
        _read_regular(root, path)[0] for path in REUSED_PINNED_PATHS
    )
    historical_rows = [
        row for row in predecessors if row.get("path") == HISTORICAL_R1_PATH
    ]
    if len(historical_rows) != 1:
        raise SuccessorProduceV5Error("P4 does not bind the historical R1 exactly once")
    historical_id, historical_r1 = _load_canonical(root, HISTORICAL_R1_PATH)
    _require_identity(historical_id, historical_rows[0], "P4 historical R1")
    if historical_r1.get("schema") != (
        "boole.native-shadow.mac3.launcher-v2-successor-producer-rehearsal.arm64.v1"
    ) or historical_r1.get("status") != "PASS-NO-IMAGE-PRODUCED":
        raise SuccessorProduceV5Error("P4 historical R1 schema or status differs")
    historical_bound = historical_r1.get("boundInputs")
    if not isinstance(historical_bound, list):
        raise SuccessorProduceV5Error("P4 historical R1 bound inputs differ")
    historical_by_path: dict[str, Mapping[str, Any]] = {}
    for row in historical_bound:
        if not isinstance(row, Mapping) or set(row) != {
            "path",
            "sha256",
            "sizeBytes",
        }:
            raise SuccessorProduceV5Error("P4 historical R1 bound input differs")
        relative = row.get("path")
        if not isinstance(relative, str):
            raise SuccessorProduceV5Error("P4 historical R1 path differs")
        if relative in historical_by_path:
            raise SuccessorProduceV5Error("P4 historical R1 repeats a bound path")
        historical_by_path[relative] = row
    import_identities: list[FileIdentity] = []
    for relative, expected in historical_by_path.items():
        if not relative.endswith(".py"):
            continue
        identity = _read_regular(root, relative)[0]
        _require_identity(identity, expected, "P4 historical Python module")
        import_identities.append(identity)
    p2_rows = [row for row in predecessors if row.get("path") == HISTORICAL_P2_PATH]
    if len(p2_rows) != 1:
        raise SuccessorProduceV5Error("P4 does not bind the historical P2 exactly once")
    p2_id, historical_p2 = _load_canonical(root, HISTORICAL_P2_PATH)
    _require_identity(p2_id, p2_rows[0], "P4 historical P2")
    if historical_p2.get("schema") != (
        "boole.native-shadow.mac3.launcher-v2-successor-production-generation-"
        "preregistration.arm64.v1"
    ) or historical_p2.get("status") != (
        "PRE-REGISTERED-PRODUCTION-GENERATION-NO-IMAGE-PRODUCTION-AUTHORITY"
    ):
        raise SuccessorProduceV5Error("P4 historical P2 schema or status differs")
    try:
        p2_reused = historical_p2["futureGeneration"]["reusedPinnedUpstream"]
    except (KeyError, TypeError) as exc:
        raise SuccessorProduceV5Error("P4 historical P2 reused bindings differ") from exc
    if not isinstance(p2_reused, list):
        raise SuccessorProduceV5Error("P4 historical P2 reused bindings differ")
    p2_reused_by_path: dict[str, Mapping[str, Any]] = {}
    for row in p2_reused:
        if not isinstance(row, Mapping) or set(row) != {
            "path",
            "role",
            "sha256",
            ...
            raise SuccessorProduceV5Error("P4 historical P2 reused binding differs")
        relative = row.get("path")
        if not isinstance(relative, str) or relative in p2_reused_by_path:
            raise SuccessorProduceV5Error("P4 historical P2 reused path differs")
        p2_reused_by_path[relative] = row
    if set(p2_reused_by_path) != set(REUSED_PINNED_PATHS):
        raise SuccessorProduceV5Error("P4 historical P2 reused path set differs")
    for identity in reused_identities:
        expected = p2_reused_by_path[identity.path]
        _require_identity(
            identity,
            {
                "path": expected["path"],
            ...
            "P4 historical P2 reused file",
        )
        import_identities.append(identity)
    expected_module_paths = {
        module_name.replace(".", "/") + ".py" for module_name in LOW_LEVEL_MODULES
    }
    actual_import_paths = {identity.path for identity in import_identities}
    missing_modules = sorted(expected_module_paths - actual_import_paths)
    if missing_modules:
        raise SuccessorProduceV5Error(
            "P4 history does not bind low-level modules: " + ", ".join(missing_modules)
        )
    return PreregisteredGeneration(
        correction=p4,
        correction_identity=p4_id,
        generation_identities=generation_identities,
        reused_identities=reused_identities,
        import_identities=tuple(import_identities),
    )
...
def verify_generation_chain(repository_root: pathlib.Path) -> GenerationChain:
    ...
    _require_absent(root, RESULT_V7_PATH, "future result-v7")
    p4_id = preregistered.correction_identity
    p4 = preregistered.correction
    v5_identities = list(preregistered.generation_identities)
    reused_identities = list(preregistered.reused_identities)
    dispatch_fence = p4_id.record()
    gate_identity = _read_regular(root, R3_GATE_PATH)[0]

    r2_id, r2 = _load_canonical(root, R3_PATH)
    r2_keys = {
        "activationAllowed", "authorisations", "bootableClaim", "boundInputs",
        "effects", "executionEnvelope", "generationFiles", "measurement",
        "predecessors", "repeatable", P4_FIELD, "reusedPinnedUpstream", "schema",
        "status",
    }
    _require_exact_keys(r2, r2_keys, "R3")
    if r2["schema"] != R3_SCHEMA or r2["status"] != R3_STATUS:
        raise SuccessorProduceV5Error("R3 schema or status differs")
    if r2["activationAllowed"] is not False or r2["bootableClaim"] is not False:
        raise SuccessorProduceV5Error("R3 claims activation or boot")
    _require_strict_equal(r2["authorisations"], ZERO_AUTHORISATIONS, "R3 authority")
    _require_strict_equal(r2["effects"], ZERO_EFFECTS, "R3 effect accounting")
    _require_strict_equal(
        r2["executionEnvelope"],
        _expected_cgroup_execution_envelope(),
        "R3 execution envelope",
    )
    _require_strict_equal(
        r2[P4_FIELD], dispatch_fence, "R3 dispatch-fence correction"
    )
    predecessors = _identity_rows((p4_id,))
    generation = _identity_rows(v5_identities)
    reused = _identity_rows(reused_identities)
    if r2["predecessors"] != predecessors:
        raise SuccessorProduceV5Error("R3 predecessors differ")
    if r2["generationFiles"] != generation:
        raise SuccessorProduceV5Error("R3 does not bind the exact five v5 files")
    if r2["reusedPinnedUpstream"] != reused:
        raise SuccessorProduceV5Error("R3 reused upstream bindings differ")
    if r2["boundInputs"] != [*predecessors, *generation, *reused]:
        raise SuccessorProduceV5Error("R3 bound-input union differs")
    if r2["repeatable"] is not True or not isinstance(r2["measurement"], dict):
        raise SuccessorProduceV5Error("R3 rehearsal result differs")
    p1_identity, p1 = _load_canonical(root, P1_PATH)
    if (
        p1_identity.sha256 != P1_SHA256
        or p1_identity.size_bytes != P1_SIZE_BYTES
    ):
        raise SuccessorProduceV5Error("P1 identity differs for R3 measurement")
    try:
    ...
    _require_strict_equal(
        r2["measurement"], expected_measurement, "R3 staging measurement"
    )

    f6_id, f6 = _load_canonical(root, F7_PATH)
    f6_keys = {
        "authorisations", "boundaries", "files", "predecessors", "rehearsalGate",
        P4_FIELD, "schema", "status", "subject", "whatThisRecordDoesNotEstablish",
    }
    _require_exact_keys(f6, f6_keys, "F7")
    if f6["schema"] != F7_SCHEMA or f6["status"] != F7_STATUS:
        raise SuccessorProduceV5Error("F7 schema or status differs")
    _require_strict_equal(f6["authorisations"], ZERO_AUTHORISATIONS, "F7 authority")
    _require_strict_equal(
        f6[P4_FIELD], dispatch_fence, "F7 dispatch-fence correction"
    )
    _require_strict_equal(
        f6["boundaries"],
        {
    ...
    if f6["files"] != generation:
        raise SuccessorProduceV5Error("F7 does not bind the exact five v5 files")
    if f6["predecessors"] != [p4_id.record(), r2_id.record()]:
        raise SuccessorProduceV5Error("F7 predecessors differ")
    if f6["rehearsalGate"] != gate_identity.record():
        raise SuccessorProduceV5Error("F7 rehearsal gate differs")
    if not isinstance(f6["subject"], str) or not f6["subject"]:
        raise SuccessorProduceV5Error("F7 subject is empty")
    if not isinstance(f6["whatThisRecordDoesNotEstablish"], list):
        raise SuccessorProduceV5Error("F7 non-claims differ")

    a7_id, a7 = _load_canonical(root, A7_PATH)
    a7_keys = {
        "authorisations", "boundaries", "grant", "predecessors", "runs", "schema",
        P4_FIELD, "status", "subject",
    }
    _require_exact_keys(a7, a7_keys, "A7")
    if a7["schema"] != A7_SCHEMA or a7["status"] != A7_STATUS:
        raise SuccessorProduceV5Error("A7 schema or status differs")
    _require_strict_equal(
        a7[P4_FIELD], dispatch_fence, "A7 dispatch-fence correction"
    )
    ...
    if not _strictly_equal(a7["authorisations"], expected_authorisations):
        raise SuccessorProduceV5Error("A7 grants more or less than one image production")
    expected_runs = dict(ZERO_RUNS, imageProductionRunsAllowed=1)
    if not _strictly_equal(a7["runs"], expected_runs):
        raise SuccessorProduceV5Error("A7 run accounting differs")
    if a7["predecessors"] != [
        p4_id.record(),
        r2_id.record(),
        f6_id.record(),
    ]:
        raise SuccessorProduceV5Error("A7 predecessors differ")
    if not isinstance(a7["subject"], str) or not a7["subject"]:
        raise SuccessorProduceV5Error("A7 subject is empty")
    grant = a7["grant"]
    if not isinstance(grant, dict) or set(grant) != {
        "attemptId", "dispatchRef", "eventName", "outputNames", "replicas",
        "resultPath", "workflowDispatchesAllowed", "workflowPath", "workflowRef",
    }:
        raise SuccessorProduceV5Error("A7 grant shape differs")
    attempt_id = grant.get("attemptId")
    if not isinstance(attempt_id, str) or re.fullmatch(
        ATTEMPT_ID_PATTERN, attempt_id
    ) is None:
        raise SuccessorProduceV5Error("A7 attempt ID violates the dispatch fence")
    dispatch_claim_ref(attempt_id)
    if grant.get("eventName") != EXACT_EVENT_NAME:
        raise SuccessorProduceV5Error("A7 grants a different event")
    if grant.get("dispatchRef") != EXACT_DISPATCH_REF:
        raise SuccessorProduceV5Error("A7 grants a different dispatch ref")
    if grant.get("workflowPath") != V5_WORKFLOW_PATH:
        raise SuccessorProduceV5Error("A7 grants a different workflow")
    if grant.get("workflowRef") != EXACT_WORKFLOW_REF:
        raise SuccessorProduceV5Error("A7 grants a different workflow ref")
    if type(grant.get("workflowDispatchesAllowed")) is not int or grant.get(
        "workflowDispatchesAllowed"
    ) != 1:
        raise SuccessorProduceV5Error("A7 dispatch count differs")
    if type(grant.get("replicas")) is not int or grant.get("replicas") != 2:
        raise SuccessorProduceV5Error("A7 replica count differs")
    if grant.get("resultPath") != RESULT_V7_PATH:
        raise SuccessorProduceV5Error("A7 result path differs")
    if grant.get("outputNames") != list(OUTPUT_NAMES):
        raise SuccessorProduceV5Error("A7 output names differ")
    if not _strictly_equal(a7["boundaries"], {
        "activationAllowed": False,
        "bootableClaim": False,
        "servingClaim": False,
    }):
        raise SuccessorProduceV5Error("A7 boundaries differ")
...
def _publish_bytes_once(path: pathlib.Path, raw: bytes, *, mode: int = 0o444) -> None:
    ...
    try:
        ...
        raise SuccessorProduceV5Error(f"output parent is absent: {parent}") from exc
    if not stat.S_ISDIR(info.st_mode) or parent.is_symlink():
        raise SuccessorProduceV5Error(f"output parent is not a real directory: {parent}")
    if os.path.lexists(path):
        raise SuccessorProduceV5Error(f"create-once output already exists: {path.name}")
...
def _open_absolute_recovery_directory(path: pathlib.Path, context: str) -> int:
    ...
    if not path.is_absolute() or ".." in path.parts:
        raise SuccessorProduceV5Error(f"{context} recovery path differs")
    descriptor: Optional[int] = None
    try:
...
def _require_recovery_identity(
    ...
    if (
        ...
        raise SuccessorProduceV5Error(f"{context} recovery identity differs")
    return value
...
def _require_recovery_directory(
    ...
    *,
    ...
    if not stat.S_ISDIR(inspected.st_mode) or not stat.S_ISDIR(opened.st_mode):
        raise SuccessorProduceV5Error(f"{context} recovery directory differs")
    ...
    if opened.st_uid != expected_uid or opened.st_gid != expected_gid:
        raise SuccessorProduceV5Error(f"{context} recovery owner differs")
    if stat.S_IMODE(opened.st_mode) != 0o700:
        raise SuccessorProduceV5Error(f"{context} recovery mode differs")
    return opened
...
def _parse_tmpfs_size_bytes(value: str) -> int:
    ...
    if match is None:
        raise SuccessorProduceV5Error("production recovery tmpfs cap differs")
    factor = {
        "": 1,
        "k": 1024,
        "m": 1024 * 1024,
...
def _require_recovery_tmpfs_caps(super_options: list[str]) -> None:
    ...
    if len(size_options) != 1 or len(inode_options) != 1:
        raise SuccessorProduceV5Error("production recovery tmpfs caps differ")
...
def _normalise_recovery_mount_identity(
    ...
    *,
    ...
    if re.fullmatch(r"boole-nsv5-[0-9a-f]{40}-r[12]", recovery_stem) is None:
        raise SuccessorProduceV5Error("production recovery stem differs")
    if not isinstance(mount_identity, Mapping):
        raise SuccessorProduceV5Error("production recovery mount differs")
    required_keys = {
        "fileSystemType",
        "majorMinor",
        "mountId",
    ...
    if set(mount_identity) != required_keys:
        raise SuccessorProduceV5Error("production recovery mount fields differ")
    ...
    if any(type(document[name]) is not str for name in scalar_names):
        raise SuccessorProduceV5Error("production recovery mount value differs")
    if document["fileSystemType"] != "tmpfs":
        raise SuccessorProduceV5Error("production recovery mount type differs")
    if re.fullmatch(r"[0-9]+:[0-9]+", document["majorMinor"]) is None:
        raise SuccessorProduceV5Error("production recovery mount device differs")
    for name in ("mountId", "parentId"):
        if re.fullmatch(r"[0-9]+", document[name]) is None:
            raise SuccessorProduceV5Error("production recovery mount id differs")
    if document["mountId"] == document["parentId"]:
        raise SuccessorProduceV5Error("production recovery mount parent differs")
    if document["mountPoint"] != str(staging_path):
        raise SuccessorProduceV5Error("production recovery mount point differs")
    if document["root"] != "/":
        raise SuccessorProduceV5Error("production recovery mount root differs")
    if document["source"] != recovery_stem:
        raise SuccessorProduceV5Error("production recovery mount source differs")
    ...
    if not {"rw", "nodev", "nosuid"}.issubset(mount_options) or {
        ...
        raise SuccessorProduceV5Error("production recovery mount options differ")
...
def _publish_recovery_record_at(
    ...
    if len(raw) > MAX_RECOVERY_RECORD_BYTES:
        raise SuccessorProduceV5Error("production recovery record exceeds byte limit")
    ...
    if members != frozenset(("staging",)):
        raise SuccessorProduceV5Error("production recovery scratch members differ")
    flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL
    if hasattr(os, "O_NOFOLLOW"):
        flags |= os.O_NOFOLLOW
    if hasattr(os, "O_CLOEXEC"):
...
def _read_recovery_record_at(
    ...
    *,
    ...
    try:
        ...
        raise SuccessorProduceV5Error("production recovery record is absent") from exc
    try:
        before = os.fstat(descriptor)
        if (
            not stat.S_ISREG(before.st_mode)
...
def discard_incomplete_production_recovery(
    *,
    ...
    if scratch.name != recovery_stem or re.fullmatch(
        r"boole-nsv5-[0-9a-f]{40}-r[12]", recovery_stem
    ) is None:
        raise SuccessorProduceV5Error("production recovery stem differs")
    parent_identity = _require_recovery_identity(
        expected_parent_identity, "output parent"
...
def publish_production_recovery_record(
    *,
    ...
    if type(expected_uid) is not int or expected_uid < 0:
        raise SuccessorProduceV5Error("production recovery uid differs")
    if type(expected_gid) is not int or expected_gid < 0:
        raise SuccessorProduceV5Error("production recovery gid differs")
    normalised_mount = _normalise_recovery_mount_identity(
        mount_identity,
        recovery_stem=recovery_stem,
...
def verify_production_recovery_record(
    *,
    ...
    try:
        ...
        raise SuccessorProduceV5Error("scratch recovery directory is absent") from exc
    scratch_identity = (scratch_stat.st_dev, scratch_stat.st_ino)
    scratch_descriptor: Optional[int] = None
    staging_descriptor: Optional[int] = None
    parent_descriptor: Optional[int] = None
...
def _mkdir_durable_once(path: pathlib.Path, *, mode: int = 0o755) -> None:
    ...
    if os.path.lexists(path):
        raise SuccessorProduceV5Error(f"fresh output directory already exists: {path}")
    try:
...
def reconcile_production_output_state(
    *,
    ...
    """Converge only known marker publication crashes without weakening A7.
...
def _make_outputs_collectable(outputs: pathlib.Path) -> None:
    ...
    if observed not in COLLECTABLE_PROGRESS_STATES:
        raise SuccessorProduceV5Error("output collectability member set differs")
...
def _write_unqualified(
    ...
    document = {
        ...
        "authoritySha256": request.chain.identities["A7"].sha256,
        ...
        "schema": (
            ...
            "unqualified-diagnostic.arm64.v5"
        ),
...
def _manifest(outputs: pathlib.Path, names: Sequence[str]) -> dict[str, str]:
    if tuple(names) != OUTPUT_NAMES:
        raise SuccessorProduceV5Error("image manifest does not name exactly three outputs")
    with _pinned_outputs(outputs, OUTPUT_NAMES) as pinned:
        result: dict[str, str] = {}
        for name in OUTPUT_NAMES:
            identity = pinned[name]
            if identity.size_bytes == 0:
                raise SuccessorProduceV5Error(f"image output is empty: {name}")
...
def _expected_readback_entry_count(repository_root: pathlib.Path) -> int:
    ...
    if identity.sha256 != P1_SHA256 or identity.size_bytes != P1_SIZE_BYTES:
        raise SuccessorProduceV5Error("P1 pinned identity differs for readback count")
    try:
    ...
    if type(staging_entries) is not int or staging_entries < 0:
        raise SuccessorProduceV5Error("P1 staging entry count differs")
...
def _validate_readback_verification(report: Any) -> None:
    ...
    if not isinstance(report, Mapping) or set(report) != expected_keys:
        raise SuccessorProduceV5Error("readback-v3 verification schema differs")
    ...
    if type(report["passed"]) is not bool:
        raise SuccessorProduceV5Error("readback-v3 verification passed differs")
    ...
    if report["passed"] is not all_ok or report["passed"] is not True:
        raise SuccessorProduceV5Error("readback-v3 verification did not pass")
...
def _validate_pending_evidence(
    ...
    if not isinstance(kernel, Mapping) or set(kernel) != {
        ...
        raise SuccessorProduceV5Error("pending kernel evidence differs")
    ...
    if not isinstance(root_disk, Mapping) or set(root_disk) != {
        ...
        raise SuccessorProduceV5Error("pending root-disk evidence differs")
    ...
    try:
        ...
        raise SuccessorProduceV5Error("pending verification differs") from exc
...
def _assert_readback_pass(
    ...
    if not isinstance(document, Mapping):
        raise SuccessorProduceV5Error("readback-v3 returned no document")
    expected_keys = {
        "activationAllowed",
        "artifactClass",
        "bootableClaim",
    ...
    if document.get("schema") != (
        ...
        raise SuccessorProduceV5Error("readback-v3 schema or release differs")
    for key in ("activationAllowed", "bootableClaim", "guestBootVerified"):
        if document.get(key) is not False:
            raise SuccessorProduceV5Error(f"readback-v3 {key} differs")
    if document.get("status") != READBACK_PASS_STATUS:
        raise SuccessorProduceV5Error("readback-v3 did not pass")
    if document.get("artifactClass") != "QUALIFIED-READBACK":
        raise SuccessorProduceV5Error("readback-v3 artifact is not qualified")
    if document.get("mayEnterQualification") is not True or document.get(
        "qualifiedForReplicaComparison"
    ) is not True:
        raise SuccessorProduceV5Error("readback-v3 barred replica comparison")
    if (
        ...
        raise SuccessorProduceV5Error("readback-v3 entry count differs")
    _require_strict_equal(
        document.get("image"),
        {
            "name": "guest-root-disk",
...
def _real_directory(path: pathlib.Path, context: str) -> pathlib.Path:
    ...
    try:
        ...
        raise SuccessorProduceV5Error(f"{context} is absent: {path}") from exc
    if not stat.S_ISDIR(info.st_mode) or path.is_symlink():
        raise SuccessorProduceV5Error(f"{context} is not a real directory: {path}")
    return path
...
def _require_empty_real_directory(path: pathlib.Path, context: str) -> None:
    ...
    try:
        ...
        if (reached.st_dev, reached.st_ino) != (before.st_dev, before.st_ino):
            raise SuccessorProduceV5Error(f"{context} identity changed")
        if os.listdir(descriptor):
            raise SuccessorProduceV5Error(f"{context} must be exactly empty")
    except SuccessorProduceV5Error:
        raise
    except OSError as exc:
        raise SuccessorProduceV5Error(f"{context} could not be inspected") from exc
    finally:
        if descriptor is not None:
            os.close(descriptor)
...
def _pinned_temporary_directory(
    ...
    with tempfile.TemporaryDirectory(prefix="tmp-v5-", dir=scratch_root) as raw:
...
def _launcher_bytes(
    ...
    try:
        ...
        raise SuccessorProduceV5Error("launcher-v2 binary is unreadable") from exc
    if not raw:
        raise SuccessorProduceV5Error(
    ...
    if require_sealed and (
        ...
        raise SuccessorProduceV5Error("launcher-v2 sealed identity differs")
    return raw
...
def _expected_cgroup_execution_envelope() -> dict[str, Any]:
    return {
        "cgroupV2": {
            ...
            "memoryHighEvents": 0,
            "memoryMaxBytes": R3_MEMORY_MAX_BYTES,
            "memoryMaxEvents": 0,
            "memoryOomEvents": 0,
            "memoryOomKillEvents": 0,
            "memorySwapMaxBytes": R3_MEMORY_SWAP_MAX_BYTES,
            "pidsMaxEvents": 0,
            "pidsMax": R3_PIDS_MAX,
        ...
        "systemdRuntimeMaxSec": {
            ...
            "requestedSeconds": R3_RUNTIME_MAX_SECONDS,
...
def _read_bounded_descriptor(fd: int, limit: int, context: str) -> bytes:
    ...
    raw = b"".join(chunks)
    if len(raw) > limit:
        raise SuccessorProduceV5Error(f"{context} exceeds its byte limit")
    return raw
...
def _read_cgroup_execution_observation(
    *,
    ...
    try:
        if re.fullmatch(
            r"boole-nsv5-rehearsal-[A-Za-z0-9]{6}\.service",
        ...
        if not stat.S_ISREG(proc_info.st_mode):
            raise SuccessorProduceV5Error("the process cgroup record is not regular")
        ...
        if not components or any(
            ...
            raise SuccessorProduceV5Error("the process cgroup-v2 path differs")
        ...
        if not stat.S_ISREG(mountinfo_info.st_mode):
            raise SuccessorProduceV5Error("the process mountinfo is not regular")
        ...
        for file_name, field_name, expected in (
            ("memory.max", "memoryMaxBytes", R3_MEMORY_MAX_BYTES),
            ("memory.swap.max", "memorySwapMaxBytes", R3_MEMORY_SWAP_MAX_BYTES),
            ("pids.max", "pidsMax", R3_PIDS_MAX),
        ):
            value_fd = -1
            try:
                value_fd = os.open(
...
def rehearse(
    *,
    ...
    for relative, context in (
        ...
        (F7_PATH, "future F7"),
        (A7_PATH, "future A7"),
        (RESULT_V7_PATH, "future result-v7"),
    ):
    ...
    if os.path.lexists(no_outputs):
        raise SuccessorProduceV5Error("rehearsal created a production output directory")
    if not isinstance(prepared, PreparedProduction):
        raise SuccessorProduceV5Error("rehearsal backend returned no prepared staging")
    predecessors = _identity_rows((preregistered.correction_identity,))
    ...
    return {
        ...
        P4_FIELD: preregistered.correction_identity.record(),
        "repeatable": True,
...
def preflight(
    *,
    ...
    """Exercise the production assembly after A7 without spending its run."""

    root = pathlib.Path(repository_root).resolve()
    chain = verify_generation_chain(root)
    verify_dispatch_repository_context(
        root,
        chain,
        github_run_id=github_run_id,
        github_run_attempt=github_run_attempt,
    ...
    with _pinned_temporary_directory(scratch_root):
        prepared = selected.prepare(request)
    if os.path.lexists(no_outputs):
        raise SuccessorProduceV5Error("preflight created a production output directory")
    if not isinstance(prepared, PreparedProduction):
        raise SuccessorProduceV5Error("preflight backend returned no prepared staging")
    return {
        "activationAllowed": False,
        "attemptId": chain.attempt_id,
        "authoritySha256": chain.identities["A7"].sha256,
...
def produce(
    *,
    ...
    if backend is None and (os.geteuid() != 0 or os.getegid() != 0):
        raise SuccessorProduceV5Error("real production backend requires root:root")
    store = _real_directory(artifact_store, "artifact store")
    scratch_root = _real_directory(scratch, "scratch root")
    _require_empty_real_directory(scratch_root, "production scratch")
    output_root = pathlib.Path(outputs)
    if os.path.lexists(output_root):
        raise SuccessorProduceV5Error("fresh output directory already exists")
    request = ProductionRequest(
    ...
    with _pinned_temporary_directory(scratch_root):
        ...
        with _consumed_attempt(request):
            ...
            if not isinstance(initrd, bytes) or not initrd:
                raise SuccessorProduceV5Error("initrd builder returned no bytes")
...
def _load_pending(
    ...
    if pathlib.Path(pending_path) != expected:
        raise SuccessorProduceV5Error("pending result path differs from the fixed name")
    ...
    try:
        ...
        raise SuccessorProduceV5Error("pending result is not JSON") from exc
    if not isinstance(document, dict) or raw != canonical_json(document):
        raise SuccessorProduceV5Error("pending result is not canonical JSON")
    expected_keys = {
        "activationAllowed",
        "attemptId",
        "authoritySha256",
    ...
    if document["schema"] != PENDING_RESULT_SCHEMA:
        raise SuccessorProduceV5Error("pending result schema differs")
    if document["status"] != PRODUCTION_PENDING_STATUS:
        raise SuccessorProduceV5Error("pending result status differs")
    if document["qualifiedForReplicaComparison"] is not False:
        raise SuccessorProduceV5Error("pending result claims qualification")
    if document["attemptId"] != chain.attempt_id:
        raise SuccessorProduceV5Error("pending result attempt differs")
    if document["authoritySha256"] != chain.identities["A7"].sha256:
        raise SuccessorProduceV5Error("pending result authority differs")
    expected_bindings = {
        name: chain.identities[name].record()
        for name in ("P4", "R3", "F7", "A7")
    }
    if document["generationBindings"] != expected_bindings:
        raise SuccessorProduceV5Error("pending result generation bindings differ")
    try:
        expected_measurement = chain.fresh_rehearsal["measurement"]
    except (KeyError, TypeError) as exc:
        raise SuccessorProduceV5Error(
            "R3 has no sealed staging measurement"
        ) from exc
    if not isinstance(expected_measurement, Mapping):
        raise SuccessorProduceV5Error("pending builder measurement differs")
    _require_strict_equal(
    ...
    if document["readbackRequired"] != READBACK_RESULT_NAME:
        raise SuccessorProduceV5Error("pending result names another readback")
    if document["activationAllowed"] is not False or document["bootableClaim"] is not False:
        raise SuccessorProduceV5Error("pending result claims activation or boot")
    if not set(OUTPUT_NAMES).issubset(pinned):
        raise SuccessorProduceV5Error("pending output pins omit an image")
    expected_manifest = [
        {"name": name, "sha256": pinned[name].sha256} for name in OUTPUT_NAMES
    ]
    if document["outputManifest"] != expected_manifest:
        raise SuccessorProduceV5Error("pending output manifest differs from image bytes")
...
def _load_canonical_output(
    ...
    *,
    ...
    try:
        ...
        raise SuccessorProduceV5Error(f"{context} is not JSON") from exc
    if not isinstance(document, dict) or raw != canonical_json(document):
        raise SuccessorProduceV5Error(f"{context} is not canonical JSON")
    return document
...
def qualify(
    *,
    ...
    if result_path != output_argument / QUALIFIED_RESULT_NAME:
        raise SuccessorProduceV5Error("qualified result path differs from the fixed name")
    root = pathlib.Path(repository_root).resolve()
    chain = verify_generation_chain(root)
    ...
    with _pinned_outputs(output_root, QUALIFICATION_PINNED_NAMES) as pinned:
        ...
        _require_strict_equal(
            marker, _marker_document(chain), "consumed marker differs from A7"
        )
        ...
        if os.path.lexists(result_path):
            ...
            if not isinstance(embedded, Mapping):
                raise SuccessorProduceV5Error("qualified result has no readback")
...
def _qualified_replica_members(outputs: pathlib.Path) -> tuple[str, ...]:
    ...
    if names != expected:
        raise SuccessorProduceV5Error("qualified replica members differ")
...
def _validate_qualified_replica(
    ...
    if set(pinned) != set(QUALIFIED_REPLICA_NAMES):
        raise SuccessorProduceV5Error("qualified replica pins differ")
    marker = _load_canonical_output(
        output_root,
        CONSUMED_MARKER_NAME,
        "consumed marker",
    ...
    *,
    ...
    with _pinned_outputs(
        ...
        if {
            ...
            raise SuccessorProduceV5Error("qualified replica contents differ")
        try:
...
def _qualified_tree_document(
    ...
    if output_root.name != "outputs":
        raise SuccessorProduceV5Error("replica provenance output path differs")
...
def replica_provenance_document(
    *,
    ...
    if type(replicas) is not int or replicas != 2:
        raise SuccessorProduceV5Error("replica provenance A7 count differs")
    if type(replica_ordinal) is not int or replica_ordinal not in (1, 2):
        raise SuccessorProduceV5Error("replica provenance ordinal differs")
    if type(strategy_job_index) is not int or strategy_job_index != replica_ordinal - 1:
        raise SuccessorProduceV5Error("replica provenance strategy index differs")
    if type(strategy_job_total) is not int or strategy_job_total != replicas:
        raise SuccessorProduceV5Error("replica provenance strategy total differs")
    if github_job != "produce":
        raise SuccessorProduceV5Error("replica provenance job differs")
    expected_artifact = REPLICA_ARTIFACT_PREFIX + str(replica_ordinal)
    if artifact_name != expected_artifact:
        raise SuccessorProduceV5Error("replica provenance artifact differs")
...
def publish_replica_provenance(
    ...
    **arguments: Any,
    ...
    if pathlib.Path(result) != expected:
        raise SuccessorProduceV5Error("replica provenance result path differs")
...
def seal_collectable_replica_bundle(
    *,
    ...
    if type(successful) is not bool:
        raise SuccessorProduceV5Error("collectable replica bundle status differs")
...
def publish_and_seal_replica_bundle(
    ...
    **arguments: Any,
    ...
    verify_dispatch_tag_object(
        ...
        head_authority_sha256=arguments["head_authority_sha256"],
    )
...
def _replica_bundle(outputs_bundle: pathlib.Path) -> tuple[pathlib.Path, pathlib.Path]:
    ...
    try:
        ...
        raise SuccessorProduceV5Error("replica bundle differs") from exc
    if set(entries) != {"outputs", REPLICA_PROVENANCE_NAME}:
        raise SuccessorProduceV5Error("replica bundle differs")
    ...
    *,
    ...
    with _pinned_outputs(
        ...
        if (left_identity.device, left_identity.inode) == (
            ...
            raise SuccessorProduceV5Error("replica provenance differs")
...
class RepositoryImageBackend:
    ...
    def _bound_python_rows(chain: Any) -> dict[str, Mapping[str, Any]]:
        ...
        imports = getattr(chain, "import_identities", ())
        rows.extend(
            identity.record() if isinstance(identity, FileIdentity) else identity
    ...
    def _controlled_imports(
        ...
        if not rows:
            raise SuccessorProduceV5Error("production chain binds no Python modules")
        ...
        bootstrap_path = _safe_path(request.repository_root, V5_PATHS[0]).resolve()
        bootstrap_row = rows.get(V5_PATHS[0])
        for name, module in tuple(sys.modules.items()):
            ...
            if name in self._owned_modules:
                ...
                if row_info is None:
                    ...
                    relative, row = V5_PATHS[0], bootstrap_row
                else:
            ...
            if (
                ...
                identity, _ = _read_regular(request.repository_root, V5_PATHS[0])
                _require_identity(
    ...
    def _pinned_json(
        ...
        if identity.sha256 != digest or identity.size_bytes != size_bytes:
            raise SuccessorProduceV5Error(f"{context} pinned identity differs")
        try:
            document = json.loads(raw.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise SuccessorProduceV5Error(f"{context} is not JSON") from exc
    ...
    def _layer_bytes(oci: pathlib.Path, receipt: Mapping[str, Any]) -> bytes:
        ...
        if not isinstance(value, str) or not value.startswith("sha256:"):
            raise SuccessorProduceV5Error("build receipt names no sha256 layer")
        digest = value.removeprefix("sha256:")
        if len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            raise SuccessorProduceV5Error("build receipt layer digest is invalid")
        relative = f"blobs/sha256/{digest}"
        identity, raw = _read_regular(oci, relative)
        if identity.sha256 != digest or not raw:
            raise SuccessorProduceV5Error("verified OCI layer identity differs")
        return raw

    @staticmethod
    def _extract_layer(layer: bytes, destination: pathlib.Path) -> int:
        if os.path.lexists(destination):
            raise SuccessorProduceV5Error("fresh extracted tree already exists")
        try:
            ...
            raise SuccessorProduceV5Error("verified OCI layer cannot be extracted") from exc
    ...
    def prepare(self, request: ProductionRequest) -> PreparedProduction:
        ...
        try:
            ...
            if sealed != source_lock:
                raise SuccessorProduceV5Error("builder parsed different source-lock bytes")
        ...
        if not hasattr(prepared, "entries") or not hasattr(prepared, "measurement"):
            raise SuccessorProduceV5Error("v3.prepare_staging returned no sealed staging")
    ...
    def _state(prepared: PreparedProduction) -> Mapping[str, Any]:
        if not isinstance(prepared.state, Mapping):
            raise SuccessorProduceV5Error("prepared repository state differs")
    ...
    def extract_kernel(
        ...
        try:
            result, disposition = kernel_module.extract(
                ...
                result_path=request.scratch / "kernel-extract-result-v5.json",
            )
        except Exception as exc:
            raise SuccessorProduceV5Error(f"kernel extraction failed: {exc}") from exc
        kernel = result.get("kernel") if isinstance(result, Mapping) else None
        if not isinstance(kernel, Mapping):
            raise SuccessorProduceV5Error("kernel extractor returned no identity")
        if disposition != "matched-the-seal":
            raise SuccessorProduceV5Error("kernel extraction disposition differs")
    ...
    def build_initrd(
        ...
        try:
            ...
            raise SuccessorProduceV5Error(f"initrd construction failed: {exc}") from exc
        if not isinstance(raw, bytes) or not raw:
            raise SuccessorProduceV5Error("initrd construction returned no bytes")
        return raw
    ...
    def _inspector_path(
        ...
        if not isinstance(rows, list):
            raise SuccessorProduceV5Error("builder authority pins no tools")
        matched = [
            row
            for row in rows
        ...
        if not isinstance(member, str) or not member.startswith("./"):
            raise SuccessorProduceV5Error("builder authority inspector path differs")
    ...
    def build_root_disk(
        ...
        try:
            ...
            raise SuccessorProduceV5Error(f"root-disk construction failed: {exc}") from exc
        if not isinstance(result, Mapping) or not isinstance(result.get("image"), Mapping):
            raise SuccessorProduceV5Error("root-disk executor returned no image identity")
        required_evidence = (
            "fsck",
            "loaderEvidence",
            "timeAudit",
    ...
    def verify_images(
        ...
        try:
            ...
            raise SuccessorProduceV5Error(f"produced image verification failed: {exc}") from exc
        if not isinstance(report, Mapping) or report.get("passed") is not True:
            raise SuccessorProduceV5Error("produced image verification did not pass")
        return dict(report)
    ...
    def readback(
        ...
        if module_file is not None:
            ...
            if pathlib.Path(module_file).resolve() != expected.resolve():
                raise SuccessorProduceV5Error("readback-v3 loaded outside verified root")
        ...
        try:
            ...
            raise SuccessorProduceV5Error(f"readback-v3 failed: {exc}") from exc
        if not isinstance(result, Mapping):
            raise SuccessorProduceV5Error("readback-v3 returned no result")
        return dict(result)


def _add_dispatch_context_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--head-authority-sha256", required=True)


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    subparsers = parser.add_subparsers(dest="mode", required=True)
    bindings_parser = subparsers.add_parser("verify-bindings")
    bindings_parser.add_argument(
        "--repository-root", type=pathlib.Path, default=REPOSITORY_ROOT
    )
    production_check_parser = subparsers.add_parser("production-check")
    production_check_parser.add_argument(
        "--repository-root", type=pathlib.Path, default=REPOSITORY_ROOT
    )
    _add_dispatch_context_arguments(production_check_parser)
    for mode in (
        ...
        _add_dispatch_context_arguments(child)
    ...
    _add_dispatch_context_arguments(qualify_parser)
    ...
    _add_dispatch_context_arguments(seal_parser)
    ...
    _add_dispatch_context_arguments(output_recovery_parser)
    for mode in (
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        if options.mode == "verify-bindings":
            ...
            print("native-shadow successor producer v5: bindings verified")
            return 0
        if options.mode == "production-check":
            ...
            verify_dispatch_repository_context(
                ...
                dispatch_ref=options.dispatch_ref,
                workflow_path=options.workflow_path,
                workflow_ref=options.workflow_ref,
                head_sha=options.head_sha,
                head_authority_sha256=options.head_authority_sha256,
            )
            print(
                "native-shadow successor producer v5: production authority verified "
                f"({chain.attempt_id})"
            )
            return 0
        if options.mode in (
            ...
            if options.mode == "publish-recovery-record":
                ...
                return 0
            print(
                f"native-shadow successor producer v5: {label} "
        ...
        if options.mode == "dispatch-claim-verify":
            ...
            print(
                "native-shadow successor producer v5: dispatch claim verified "
                f"({chain.attempt_id})"
            )
            return 0
        if options.mode == "qualify":
            ...
            print(
                "native-shadow successor producer v5: qualify PASS "
                f"sha256={_sha256(canonical_json(document))}"
            )
            return 0
        if options.mode == "seal-replica-bundle":
            ...
            print("native-shadow successor producer v5: replica bundle sealed")
            return 0
        ...
        if options.mode in (
            ...
            print(
                f"native-shadow successor producer v5: {label} PASS "
                f"sha256={_sha256(canonical_json(document))}"
            )
            return 0
        common = {
        ...
        print(
            f"native-shadow successor producer v5: {options.mode} PASS "
            f"sha256={_sha256(canonical_json(document))}"
        )
        return 0
    except SuccessorProduceV5Error as exc:
        print(f"native-shadow successor producer v5: FAIL: {exc}", file=sys.stderr)
        return 1
