...
MINING_V2_TARGET_FAMILY = "boole.calibration.pow.v2"
MINING_V3_TARGET_FAMILY = "boole.calibration.pow.v3"
...
def target_family_for_mode(benchmark_mode: str) -> str:
    ...
    if benchmark_mode == "mining-v2":
        return MINING_V2_TARGET_FAMILY
    if benchmark_mode == "mining-v3":
        return MINING_V3_TARGET_FAMILY
    raise SystemExit(f"unsupported benchmark mode: {benchmark_mode}")


# --- v031-lp generator (Slice S8) -------------------------------------------
#
# Port of the lp-only arm of pof's `Boole.Family.GenTargetCatalogV031`.
# Cursor advance, op-kind selection, and chooseInt formulae mirror pof's
# Lean implementation byte-for-byte so the family identity is preserved.

_V031LP_OPS = ("mapAdd", "mapMul", "sortAsc")


def _v031lp_cursor_read(buf: bytes, pos: int, mod_n: int) -> tuple[int, int]:
    """Mirror of pof's `Cursor.readNat`: read one byte, return `(byte % mod_n, pos+1)`.
    Cursor wraps modulo `len(buf)` to match the Lean semantics."""
    if mod_n == 0 or len(buf) == 0:
        return 0, pos
    byte = buf[pos % len(buf)]
    return (byte % mod_n), (pos + 1)


def _v031lp_choose_int(buf: bytes, pos: int, range_n: int) -> tuple[int, int]:
    """Mirror of pof's `Cursor.chooseInt`: pick `n ∈ [0, 2*range_n]`, return
    `n - range_n` ∈ [-range_n, range_n]."""
    n, pos = _v031lp_cursor_read(buf, pos, range_n * 2 + 1)
    return n - range_n, pos


def _v031lp_gen_op_length(buf: bytes, pos: int) -> tuple[dict[str, Any], int]:
    """Mirror of pof's `genOpV031Length`: 3-way pick over the
    length-preserving op family. mapAdd k draws k via chooseInt 9; mapMul k
    draws k via readNat 5 + 1; sortAsc takes no argument."""
    i, pos = _v031lp_cursor_read(buf, pos, 3)
    if i == 0:
        k, pos = _v031lp_choose_int(buf, pos, 9)
        return {"op": "mapAdd", "k": k}, pos
    if i == 1:
        k_raw, pos = _v031lp_cursor_read(buf, pos, 5)
        return {"op": "mapMul", "k": k_raw + 1}, pos
    return {"op": "sortAsc"}, pos


def _v031lp_generate(seed_bytes: bytes) -> dict[str, Any]:
    """Generate the full lp-only N=1 target. Returns chain + cursor-derived
    fields. The cursor also consumes the pred / lenRaw / noiseRaw positions
    pof's full v031 generator reads after the chain so the cursor model
    stays byte-compatible if v2 ever extends to N≥2 mixed branches."""
    pos = 0
    d_raw, pos = _v031lp_cursor_read(seed_bytes, pos, 6)
    chain_len = (d_raw % 6) + 1
    chain: list[dict[str, Any]] = []
    for _ in range(chain_len):
        op, pos = _v031lp_gen_op_length(seed_bytes, pos)
        chain.append(op)
    # Advance through the cursor positions pof's full v031 reads after
    # the chain (pred / inputLen / noiseDefs). Results unused at N=1
    # lp-only but byte-consumed for forward compatibility.
    _, pos = _v031lp_cursor_read(seed_bytes, pos, 6)   # pred kind
    _, pos = _v031lp_cursor_read(seed_bytes, pos, 5)   # inputLen
    _, pos = _v031lp_cursor_read(seed_bytes, pos, 5)   # noiseDefs
    return {"D": d_raw, "chainLen": chain_len, "chain": chain}


def _v031lp_op_apply(op: dict[str, Any], inner: str) -> str:
    """Render op application onto a Lean source expression. Mirrors pof's
    `Op.applyTo`."""
    if op["op"] == "mapAdd":
        return f"(mapAdd ({op['k']} : Int) {inner})"
    if op["op"] == "mapMul":
        return f"(mapMul ({op['k']} : Int) {inner})"
    if op["op"] == "sortAsc":
        return f"(sortAsc {inner})"
    raise SystemExit(f"unknown op kind: {op['op']!r}")


def _v031lp_render_chain_expr(chain: list[dict[str, Any]], var: str = "xs") -> str:
    """Fold the chain onto `var` via successive `Op.applyTo`."""
    expr = var
    for op in chain:
        expr = _v031lp_op_apply(op, expr)
    return expr


def _v031lp_length_lemma_call(op: dict[str, Any], inner: str) -> str:
    """Render a single `length_<op>` lemma application. `inner` is the
    expression passed to the lemma (either `xs` for the innermost / final
    position, or `_` for non-final)."""
    if op["op"] == "mapAdd":
        return f"length_mapAdd ({op['k']} : Int) {inner}"
    if op["op"] == "mapMul":
        return f"length_mapMul ({op['k']} : Int) {inner}"
    if op["op"] == "sortAsc":
        return f"length_sortAsc {inner}"
    raise SystemExit(f"unknown op kind: {op['op']!r}")


def _v031lp_render_canonical_proof(chain: list[dict[str, Any]]) -> str:
    """Emit a canonical proof term for the lp goal `(<chain xs>).length =
    xs.length`. Shape mirrors `projects/pof/lean/Boole/Family/V0ProbeV031.lean`:

    * chainLen = 1: `fun xs => length_<op_0> <args> xs`.
    * chainLen ≥ 2: `fun xs => (length_<op_{n-1}> _).trans
      ((length_<op_{n-2}> _).trans (... (length_<op_0> <args> xs)))`.
    """
    if not chain:
        # Empty chain reduces to `length_id xs` — but the generator
        # guarantees chainLen ≥ 1, so we don't expect to hit this.
        return "fun xs => rfl"
    n = len(chain)
    # Innermost-to-outermost: chain[0] is applied first, so its lemma takes
    # `xs` directly; subsequent positions take `_`.
    innermost = _v031lp_length_lemma_call(chain[0], "xs")
    if n == 1:
        return f"fun xs => {innermost}"
    body = innermost
    # Wrap from inside out: chain[1], chain[2], ..., chain[n-1].
    # Each wrap is `(length_<op> _).trans (<body>)`.
    for op in chain[1:]:
        outer = _v031lp_length_lemma_call(op, "_")
        body = f"({outer}).trans ({body})"
    return f"fun xs => {body}"


# --- v031-mixed generator (Slice S8b) ---------------------------------------
#
# Port of pof's full `Boole.Family.GenTargetCatalogV031` 5-way invariant
# generator, narrowed to N=1 for now. Cursor read order matches pof's
# `genBranchV031` byte-for-byte: invClass selector → optional predicate
# sub-cursor → body ops → trailing pred / lenRaw / noiseRaw. Boole prepends
# a single `D` byte (matches v2's design) so the seed determines difficulty
# without an external parameter.

_V031_INV_CLASSES = (
    "allSatisfy", "sortedAsc", "dedupFirst", "partitionEq", "lengthPreserved",
)
_V031_TRUTHY_OPS = ("filterP", "mapAdd", "mapMul", "dedup", "sortAsc")
_V031_INV_TO_WITNESS_OP = {
    "allSatisfy": "filterP",
    "sortedAsc": "sortAsc",
    "dedupFirst": "dedup",
    "partitionEq": None,
    "lengthPreserved": None,
}
_V031_INV_TO_WITNESS_LEMMA = {
    "allSatisfy": "all_filterByPred_self",
    "sortedAsc": "pairwise_sortAsc",
    "dedupFirst": "nodup_dedup",
    "partitionEq": "partition_eq_filter_filter",
    "lengthPreserved": None,  # composed via length_* + Eq.trans
}


def _v031mixed_gen_pred(buf: bytes, pos: int) -> tuple[dict[str, Any], int]:
    """Mirror of pof's `genPredV031` — 6-way pick over the predicate
    family. `modK` consumes 3 sub-bytes (kind + k + r); the others 1–2."""
    i, pos = _v031lp_cursor_read(buf, pos, 6)
    if i == 0:
        return {"kind": "even"}, pos
    if i == 1:
        return {"kind": "odd"}, pos
    if i == 2:
        k, pos = _v031lp_choose_int(buf, pos, 9)
        return {"kind": "ltK", "k": k}, pos
    if i == 3:
        k, pos = _v031lp_choose_int(buf, pos, 9)
        return {"kind": "gtK", "k": k}, pos
    if i == 4:
        k, pos = _v031lp_choose_int(buf, pos, 9)
        return {"kind": "eqK", "k": k}, pos
    k_raw, pos = _v031lp_cursor_read(buf, pos, 5)
    k = k_raw + 2
    r_raw, pos = _v031lp_cursor_read(buf, pos, 6)
    r = r_raw
    return {"kind": "modK", "k": k, "r": r}, pos


def _v031mixed_gen_op(buf: bytes, pos: int) -> tuple[dict[str, Any], int]:
    """Mirror of pof's `genOpV031` — full v0.2 5-set: filterP / mapAdd /
    mapMul / dedup / sort. `mapMul` reads `readNat 5 + 1` so k ∈ [1, 5];
    `mapAdd` reads `chooseInt 9` so k ∈ [-9, 9]."""
    i, pos = _v031lp_cursor_read(buf, pos, 5)
    if i == 0:
        pred, pos = _v031mixed_gen_pred(buf, pos)
        return {"op": "filterP", "pred": pred}, pos
    if i == 1:
        k, pos = _v031lp_choose_int(buf, pos, 9)
        return {"op": "mapAdd", "k": k}, pos
    if i == 2:
...
def _v031mixed_gen_inv_class(buf: bytes, pos: int) -> tuple[dict[str, Any], int]:
    """Mirror of pof's `genInvClassV031` — 5-way pick. `allSatisfy` and
    `partitionEq` consume an extra predicate sub-cursor."""
    i, pos = _v031lp_cursor_read(buf, pos, 5)
    if i == 0:
        pred, pos = _v031mixed_gen_pred(buf, pos)
        return {"kind": "allSatisfy", "pred": pred}, pos
    if i == 1:
        return {"kind": "sortedAsc"}, pos
    if i == 2:
        return {"kind": "dedupFirst"}, pos
    if i == 3:
        pred, pos = _v031mixed_gen_pred(buf, pos)
        return {"kind": "partitionEq", "pred": pred}, pos
    return {"kind": "lengthPreserved"}, pos


def _v031mixed_witness_op(inv_class: dict[str, Any]) -> dict[str, Any] | None:
    """Mirror of pof's `witnessOpsV031`: pick the single witness op (or
    None) that gets appended at the chain end for each invariant class."""
    kind = inv_class["kind"]
    if kind == "allSatisfy":
        return {"op": "filterP", "pred": inv_class["pred"]}
    if kind == "sortedAsc":
        return {"op": "sortAsc"}
    if kind == "dedupFirst":
        return {"op": "dedup"}
...
def _v031mixed_gen_branch(chain_len: int, buf: bytes, pos: int) -> tuple[dict[str, Any], int]:
    """Mirror of pof's `genBranchV031`. Returns
    `({invClass, bodyChain, witnessOp, branchChain}, pos)`."""
    inv_class, pos = _v031mixed_gen_inv_class(buf, pos)
    kind = inv_class["kind"]
    if kind == "lengthPreserved":
        body: list[dict[str, Any]] = []
        for _ in range(chain_len):
            op, pos = _v031lp_gen_op_length(buf, pos)
            body.append(op)
        witness = None
        branch = list(body)
    else:
        witness = _v031mixed_witness_op(inv_class)
        body_len = chain_len - (1 if witness is not None else 0)
        body_len = max(0, body_len)
        body = []
        for _ in range(body_len):
            op, pos = _v031mixed_gen_op(buf, pos)
            body.append(op)
        branch = list(body) + ([witness] if witness is not None else [])
    return {
        "invClass": inv_class,
        "bodyChain": body,
        "witnessOp": witness,
...
def _v031mixed_generate(seed_bytes: bytes) -> dict[str, Any]:
    """Top-level mixed-family generator (N=1). Boole-side prepended `D` byte
    matches v2's design. Cursor advances through trailing pred/lenRaw/noiseRaw
    bytes for forward-compat alignment with a future N≥2 extension."""
    pos = 0
    d_raw, pos = _v031lp_cursor_read(seed_bytes, pos, 6)
    chain_len = (d_raw % 6) + 1
    branch, pos = _v031mixed_gen_branch(chain_len, seed_bytes, pos)
    # Forward-compat trailing positions (mirror pof's GenTargetCatalogV031Lp).
    _, pos = _v031mixed_gen_pred(seed_bytes, pos)        # legacy pred
    _, pos = _v031lp_cursor_read(seed_bytes, pos, 5)     # inputLen
    _, pos = _v031lp_cursor_read(seed_bytes, pos, 5)     # noiseDefs
    return {
        "D": d_raw,
        "chainLen": chain_len,
        "invClass": branch["invClass"],
        "bodyChain": branch["bodyChain"],
        "witnessOp": branch["witnessOp"],
        "branchChain": branch["branchChain"],
    }


def _v031mixed_render_int(k: int) -> str:
    return f"({k} : Int)"


def _v031mixed_render_pred_to_bool(pred: dict[str, Any]) -> str:
    """Mirror of pof's `Pred.toBoolFn`: render a `Pred` as a Lean
    `Int → Bool` source expression."""
    kind = pred["kind"]
    if kind == "even":
        return "(fun x : Int => x % 2 == 0)"
    if kind == "odd":
        return "(fun x : Int => x % 2 != 0)"
    if kind == "ltK":
        return f"(fun x : Int => decide (x < {_v031mixed_render_int(pred['k'])}))"
    if kind == "gtK":
        return f"(fun x : Int => decide ({_v031mixed_render_int(pred['k'])} < x))"
    if kind == "eqK":
        return f"(fun x : Int => x == {_v031mixed_render_int(pred['k'])})"
    if kind == "modK":
        return (
            f"(fun x : Int => x % {_v031mixed_render_int(pred['k'])} "
            f"== {_v031mixed_render_int(pred['r'])})"
        )
    raise SystemExit(f"unknown pred kind: {kind!r}")


def _v031mixed_render_pred_to_bool_not(pred: dict[str, Any]) -> str:
    base = _v031mixed_render_pred_to_bool(pred)
    return f"(fun y : Int => !({base} y))"


def _v031mixed_render_op_apply(op: dict[str, Any], inner: str) -> str:
    """Mirror of pof's `Op.applyTo` for the full v0.2 op set."""
    kind = op["op"]
    if kind == "filterP":
        pred = _v031mixed_render_pred_to_bool(op["pred"])
        return f"(filterByPred {pred} {inner})"
    if kind == "mapAdd":
        return f"(mapAdd {_v031mixed_render_int(op['k'])} {inner})"
    if kind == "mapMul":
        return f"(mapMul {_v031mixed_render_int(op['k'])} {inner})"
    if kind == "dedup":
        return f"(dedup {inner})"
    if kind == "sortAsc":
        return f"(sortAsc {inner})"
    raise SystemExit(f"unknown op kind: {kind!r}")


def _v031mixed_render_chain_expr(chain: list[dict[str, Any]], var: str = "xs") -> str:
    """Fold `_v031mixed_render_op_apply` over a chain. Used for both the
    body chain and the full result (body + witness) chain."""
    expr = var
    for op in chain:
        expr = _v031mixed_render_op_apply(op, expr)
    return expr


def _v031mixed_render_theorem_rhs(inv_class: dict[str, Any], result_expr: str) -> str:
    """Mirror of pof's `InvariantClassV031.toProp` against the
    full-result expression (body + witness applied to xs)."""
    kind = inv_class["kind"]
    if kind == "allSatisfy":
        pbool = _v031mixed_render_pred_to_bool(inv_class["pred"])
        return f"({result_expr}).all {pbool} = true"
    if kind == "sortedAsc":
        return f"List.Pairwise (· ≤ ·) {result_expr}"
    if kind == "dedupFirst":
        return f"List.Nodup {result_expr}"
    if kind == "partitionEq":
        pbool = _v031mixed_render_pred_to_bool(inv_class["pred"])
        pnot = _v031mixed_render_pred_to_bool_not(inv_class["pred"])
        return (
            f"({result_expr}).partition {pbool} = "
            f"(({result_expr}).filter {pbool}, ({result_expr}).filter {pnot})"
        )
    if kind == "lengthPreserved":
        return f"({result_expr}).length = xs.length"
    raise SystemExit(f"unknown invariant kind: {kind!r}")


def _v031mixed_render_canonical_proof(
    inv_class: dict[str, Any],
    body_chain: list[dict[str, Any]],
    body_expr: str,
) -> str:
    """Emit the canonical proof term per invariant class.

    For the four witness branches the proof is a single witness-lemma
    application against `body_expr` (the chain *before* the witness op
    is appended — the witness lemma's signature handles the witness
    op internally). For `lengthPreserved` we delegate to the existing
    v2 proof shape, which composes `length_*` lemmas via `Eq.trans`.
    """
    kind = inv_class["kind"]
    if kind == "allSatisfy":
        pbool = _v031mixed_render_pred_to_bool(inv_class["pred"])
        return f"fun xs => all_filterByPred_self {pbool} {body_expr}"
    if kind == "sortedAsc":
        return f"fun xs => pairwise_sortAsc {body_expr}"
    if kind == "dedupFirst":
        return f"fun xs => nodup_dedup {body_expr}"
    if kind == "partitionEq":
        pbool = _v031mixed_render_pred_to_bool(inv_class["pred"])
        return f"fun xs => partition_eq_filter_filter {pbool} {body_expr}"
    if kind == "lengthPreserved":
        # Delegate to the v2 emitter — same chain shape, same proof tree.
        return _v031lp_render_canonical_proof(body_chain)
    raise SystemExit(f"unknown invariant kind: {kind!r}")
...
def attempt_context(run_id: str, target: str, attempt_index: int, *, benchmark_mode: str = "mining") -> dict[str, Any]:
    ...
    if benchmark_mode == "mining-v2":
        cursor_seed = hashlib.sha256((seed + "|v031-lp-cursor").encode("utf-8")).digest()
        gen = _v031lp_generate(cursor_seed)
        chain_expr = _v031lp_render_chain_expr(gen["chain"])
        canonical_proof = _v031lp_render_canonical_proof(gen["chain"])
        theorem_name = f"boole_benchmark_pow_v2_target_{attempt_index + 1}"
        return {
            ...
            "lpChain": gen["chain"],
            "lpChainLen": gen["chainLen"],
            "lpD": gen["D"],
            "chainExpr": chain_expr,
            "canonicalProof": canonical_proof,
        }
    if benchmark_mode == "mining-v3":
        cursor_seed = hashlib.sha256((seed + "|v031-mixed-cursor").encode("utf-8")).digest()
        gen = _v031mixed_generate(cursor_seed)
        body_expr = _v031mixed_render_chain_expr(gen["bodyChain"])
        result_expr = _v031mixed_render_chain_expr(gen["branchChain"])
        theorem_rhs = _v031mixed_render_theorem_rhs(gen["invClass"], result_expr)
        canonical_proof = _v031mixed_render_canonical_proof(
            gen["invClass"], gen["bodyChain"], body_expr,
        )
        theorem_name = f"boole_benchmark_pow_v3_target_{attempt_index + 1}"
        return {
            "benchmarkMode": benchmark_mode,
            "targetFamily": target_family,
            "attemptIndex": attempt_index,
            ...
            "nonce": nonce,
            "theoremName": theorem_name,
            "invariantClass": gen["invClass"]["kind"],
            "invariantPred": gen["invClass"].get("pred"),
            "branchChain": gen["branchChain"],
            "bodyChain": gen["bodyChain"],
            "witnessOp": gen["witnessOp"],
            "chainLen": gen["chainLen"],
            "D": gen["D"],
            "bodyExpr": body_expr,
            "resultExpr": result_expr,
            "theoremRhs": theorem_rhs,
            "canonicalProof": canonical_proof,
        }
    return {
        "benchmarkMode": benchmark_mode,
...
def wrap_proof_term_candidate(proof_term: str, *, benchmark_mode: str = "mining", attempt_context: dict[str, Any] | None = None) -> str:
    ...
    if benchmark_mode == "mining-v2":
        ctx = attempt_context or globals()["attempt_context"]("manual", "manual", 0, benchmark_mode="mining-v2")
        theorem_name = ctx["theoremName"]
        challenge = ctx["challenge"]
        nonce = ctx["nonce"]
        chain_expr = ctx["chainExpr"]
        chain_len = ctx["lpChainLen"]
        d_raw = ctx["lpD"]
        return (
            f"-- benchmarkMode: mining-v2\n"
            f"-- targetFamily: {MINING_V2_TARGET_FAMILY}\n"
            f"-- lotteryChallenge: {challenge}\n"
            f"-- lotteryNonce: {nonce}\n"
            f"-- lpChainLen: {chain_len}\n"
            f"-- lpD: {d_raw}\n"
            f"import Boole.Family.V0Helpers\n"
            f"\n"
            f"namespace {theorem_name}\n"
            f"\n"
            f"open Boole.Family.V0Helpers\n"
            f"\n"
            f"theorem {theorem_name} : ∀ (xs : List Int),\n"
            f"    {chain_expr}.length = xs.length :=\n"
            f"{indented}\n"
            f"\n"
            f"end {theorem_name}\n"
        )
    if benchmark_mode == "mining-v3":
        ctx = attempt_context or globals()["attempt_context"]("manual", "manual", 0, benchmark_mode="mining-v3")
        theorem_name = ctx["theoremName"]
        challenge = ctx["challenge"]
        nonce = ctx["nonce"]
        invariant_class = ctx["invariantClass"]
        chain_len = ctx["chainLen"]
        d_raw = ctx["D"]
        theorem_rhs = ctx["theoremRhs"]
        return (
            f"-- benchmarkMode: mining-v3\n"
            f"-- targetFamily: {MINING_V3_TARGET_FAMILY}\n"
            f"-- lotteryChallenge: {challenge}\n"
            f"-- lotteryNonce: {nonce}\n"
            f"-- invariantClass: {invariant_class}\n"
            f"-- chainLen: {chain_len}\n"
            f"-- D: {d_raw}\n"
            f"import Boole.Family.V0Helpers\n"
            f"\n"
            f"namespace {theorem_name}\n"
...
def model_proof_term_prompt(*, benchmark_mode: str = "mining", attempt_context: dict[str, Any] | None = None) -> str:
    ...
    if benchmark_mode == "mining-v2":
        ...
        chain_expr = ctx["chainExpr"]
        return (
            "Boole proof-to-block benchmark MINING-V2 target contract (v031-lp family). "
            "Return exactly one Lean 4 proof term for this theorem body, not a full theorem and not a Markdown code block. "
            f"Target family: `{ctx['targetFamily']}`. Lottery challenge: `{ctx['challenge']}`. Nonce: `{ctx['nonce']}`. "
            f"Target theorem: `theorem {ctx['theoremName']} : ∀ (xs : List Int), {chain_expr}.length = xs.length := <YOUR_PROOF_TERM>`. "
            "The world imports `Boole.Family.V0Helpers` and opens its namespace, so the helpers `mapAdd`, `mapMul`, `sortAsc` "
            "and the length-preservation lemmas `length_mapAdd`, `length_mapMul`, `length_sortAsc` are in scope unqualified. "
            "Close the goal by composing `length_*` lemmas via `Eq.trans`. "
            "Worked examples: chain `mapAdd 3 xs` closes by `fun xs => length_mapAdd 3 xs`; "
            "chain `sortAsc (mapMul 2 (mapAdd 3 xs))` closes by "
            "`fun xs => (length_sortAsc _).trans ((length_mapMul 2 _).trans (length_mapAdd 3 xs))`. "
            "Do not include `theorem`, `lemma`, `example`, `import`, explanations, JSON, markdown fences, `by`, `sorry`, or `admit`. "
    ...
    if benchmark_mode == "mining-v3":
        ctx = attempt_context or globals()["attempt_context"]("manual", "manual", 0, benchmark_mode="mining-v3")
        return (
            "Boole proof-to-block benchmark MINING-V3 target contract (v031 mixed 5-way family). "
            "Return exactly one Lean 4 proof term for this theorem body, not a full theorem and not a Markdown code block. "
            f"Target family: `{ctx['targetFamily']}`. Lottery challenge: `{ctx['challenge']}`. Nonce: `{ctx['nonce']}`. "
            f"Target theorem: `theorem {ctx['theoremName']} : ∀ (xs : List Int), {ctx['theoremRhs']} := <YOUR_PROOF_TERM>`. "
            f"Per-attempt invariant class: `{ctx['invariantClass']}` (one of "
            "`allSatisfy`, `sortedAsc`, `dedupFirst`, `partitionEq`, `lengthPreserved`). "
            "The world imports `Boole.Family.V0Helpers` and opens its namespace, so the op helpers "
            "`filterByPred`, `mapAdd`, `mapMul`, `dedup`, `sortAsc` are in scope unqualified, along with "
            "the witness lemmas `all_filterByPred_self` (allSatisfy), `pairwise_sortAsc` (sortedAsc), "
            "`nodup_dedup` (dedupFirst), `partition_eq_filter_filter` (partitionEq), and the length lemmas "
            "`length_mapAdd`, `length_mapMul`, `length_sortAsc` (lengthPreserved, composed via `Eq.trans`). "
            "Worked examples (one per branch): "
            "(allSatisfy) `(filterByPred p xs).all p = true` closes by `fun xs => all_filterByPred_self p xs`; "
            "(sortedAsc) `List.Pairwise (· ≤ ·) (sortAsc xs)` closes by `fun xs => pairwise_sortAsc xs`; "
            "(dedupFirst) `List.Nodup (dedup xs)` closes by `fun xs => nodup_dedup xs`; "
            "(partitionEq) `xs.partition p = (xs.filter p, xs.filter (fun x => !(p x)))` closes by "
            "`fun xs => partition_eq_filter_filter p xs`; "
            "(lengthPreserved) `(sortAsc (mapMul 2 (mapAdd 3 xs))).length = xs.length` closes by "
            "`fun xs => (length_sortAsc _).trans ((length_mapMul 2 _).trans (length_mapAdd 3 xs))`. "
            "Do not include `theorem`, `lemma`, `example`, `import`, explanations, JSON, markdown fences, `by`, `sorry`, or `admit`. "
            "The returned term will be bound to this per-attempt lottery sample and verified by Boole's submit-lean path."
        )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--benchmark-mode", choices=["mining", "mining-v2", "mining-v3", "smoke"], default="mining", help="Benchmark target mode. mining=per-attempt v031-lp calibrated target family (legacy v1); mining-v2=v031-lp lengthPreserved family (boole.calibration.pow.v2); mining-v3=v031 mixed 5-way family (boole.calibration.pow.v3, allSatisfy/sortedAsc/dedupFirst/partitionEq/lengthPreserved); smoke=True.intro pipeline-only.")
