...
from scripts import native_shadow_successor_produce_phase_arm64_v5 as phase


P4_RELATIVE = phase.P4_PATH
...
def cgroup_observation() -> tuple[str, dict[str, int]]:
    return (
        "/system.slice/boole-nsv5-rehearsal-ABC123.service",
        {
            "memoryHighEvents": 0,
            "memoryMaxBytes": 8 * 1024 * 1024 * 1024,
            "memoryMaxEvents": 0,
...
class GenerationChainTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        p4 = json.loads((REPOSITORY_ROOT / P4_RELATIVE).read_text())
        pinned_rows = [
            *p4["historicalGeneration"]["files"],
            *p4["predecessors"],
        ]
        for relative in dict.fromkeys(row["path"] for row in pinned_rows):
            destination = self.root / relative
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(REPOSITORY_ROOT / relative, destination)
        historical_r1 = json.loads(
            (REPOSITORY_ROOT / phase.HISTORICAL_R1_PATH).read_text()
        )
        for row in historical_r1["boundInputs"]:
            relative = row["path"]
            ...
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(REPOSITORY_ROOT / relative, destination)
        p4_destination = self.root / P4_RELATIVE
        p4_destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(REPOSITORY_ROOT / P4_RELATIVE, p4_destination)
        for relative in (
            phase.P1_PATH,
            ...
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(REPOSITORY_ROOT / relative, destination)
        for relative in phase.V5_PATHS:
            destination = self.root / relative
            destination.parent.mkdir(parents=True, exist_ok=True)
        ...
        for relative in phase.REUSED_PINNED_PATHS:
            ...
            if not destination.exists():
                destination.parent.mkdir(parents=True, exist_ok=True)
                shutil.copyfile(REPOSITORY_ROOT / relative, destination)
        gate = self.root / phase.R3_GATE_PATH
    ...
    def write_chain(self) -> None:
        ...
        r2 = {
            ...
            "schema": phase.R3_SCHEMA,
            "status": phase.R3_STATUS,
        }
        r3_path = self.root / phase.R3_PATH
        r3_path.parent.mkdir(parents=True, exist_ok=True)
        r3_path.write_bytes(canonical(r2))
        r3_identity = identity(self.root, phase.R3_PATH)
        f6 = {
            "authorisations": dict(phase.ZERO_AUTHORISATIONS),
            "boundaries": {
                "activationAllowed": False,
            ...
            "files": generation,
            "predecessors": [p4, r3_identity],
            phase.P4_FIELD: p4,
            "rehearsalGate": identity(self.root, phase.R3_GATE_PATH),
            "schema": phase.F7_SCHEMA,
            "status": phase.F7_STATUS,
            "subject": "Pin the exact production-only v4 generation after fresh R3.",
            "whatThisRecordDoesNotEstablish": [
                "image production authority",
                "guest boot authority",
            ],
        }
        f7_path = self.root / phase.F7_PATH
        f7_path.write_bytes(canonical(f6))
        f7_identity = identity(self.root, phase.F7_PATH)
        a6 = {
            "authorisations": dict(
            ...
            "grant": {
                "attemptId": "mac3-launcher-v2-successor-v5-attempt-1",
                "dispatchRef": phase.EXACT_DISPATCH_REF,
                "eventName": phase.EXACT_EVENT_NAME,
                "outputNames": list(phase.OUTPUT_NAMES),
                "replicas": 2,
                "resultPath": phase.RESULT_V7_PATH,
                "workflowDispatchesAllowed": 1,
                "workflowPath": phase.V5_WORKFLOW_PATH,
                "workflowRef": phase.EXACT_WORKFLOW_REF,
            },
            "predecessors": [p4, r3_identity, f7_identity],
            phase.P4_FIELD: p4,
            "runs": dict(phase.ZERO_RUNS, imageProductionRunsAllowed=1),
            "schema": phase.A7_SCHEMA,
            "status": phase.A7_STATUS,
            "subject": "Grant exactly one named production dispatch and no boot.",
        }
        (self.root / phase.A7_PATH).write_bytes(canonical(a6))
    ...
    def synthetic_dispatch_tag_fixture(self):
        ...
        run_id = "33299900001"
        head_sha = "e" * 40
        head_authority_sha256 = chain.identities["A7"].sha256
        claim_ref = phase.dispatch_claim_ref(chain.attempt_id)
        message = phase.dispatch_claim_message(
            chain,
            github_run_id=run_id,
            github_run_attempt="1",
            event_name=phase.EXACT_EVENT_NAME,
            dispatch_ref=phase.EXACT_DISPATCH_REF,
            workflow_path=phase.V5_WORKFLOW_PATH,
            workflow_ref=phase.EXACT_WORKFLOW_REF,
            head_sha=head_sha,
    ...
    def dispatch_tag_fixture(self):
        ...
        if not (self.root / ".git").exists():
            ...
            subprocess.run(
                [git, "-C", str(self.root), "add", "--", phase.A7_PATH],
                check=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
    ...
    def provenanced_replica_bundles(self):
        ...
        for ordinal, source in ((1, left), (2, right)):
            ...
            document = phase.replica_provenance_document(
                ...
                github_job="produce",
                artifact_name=f"native-shadow-successor-v5-replica-{ordinal}",
                **dispatch,
            )
    ...
    def test_strict_chain_reaches_one_named_unrun_authority(self) -> None:
        ...
        self.assertEqual(
            chain.attempt_id, "mac3-launcher-v2-successor-v5-attempt-1"
        )
    ...
    def test_preregistered_generation_requires_the_dispatch_fence_correction(self) -> None:
        (self.root / P4_RELATIVE).unlink()

        with self.assertRaisesRegex(phase.SuccessorProduceV5Error, "P4"):
    ...
    def test_preregistered_generation_rejects_a_tampered_dispatch_fence(self) -> None:
        correction = self.root / P4_RELATIVE
    ...
    def test_r2_must_directly_bind_the_dispatch_fence_correction(self) -> None:
        self.write_chain()
        r3_path = self.root / phase.R3_PATH
        r2 = json.loads(r3_path.read_text())
        del r2[phase.P4_FIELD]
        r3_path.write_bytes(canonical(r2))

        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "R3 keys differ"
        ):
    ...
    def test_r2_rejects_a_requested_cap_masquerading_as_kernel_observation(
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "R3 execution envelope"
        ):
            phase.verify_generation_chain(self.root)
    ...
    def test_live_cgroup_v2_caps_are_read_from_the_kernel_files_not_arguments(
        ...
        unit_name = "boole-nsv5-rehearsal-ABC123.service"
    ...
    def test_a6_attempt_id_must_match_the_dispatch_fence_pattern(self) -> None:
        self.write_chain()
        authority_path = self.root / phase.A7_PATH
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "A7 attempt ID"
        ):
            phase.verify_generation_chain(self.root)
    ...
    def test_f6_must_directly_bind_the_dispatch_fence_correction(self) -> None:
        self.write_chain()
        f7_path = self.root / phase.F7_PATH
        f6 = json.loads(f7_path.read_text())
        del f6[phase.P4_FIELD]
        f7_path.write_bytes(canonical(f6))

        with self.assertRaisesRegex(phase.SuccessorProduceV5Error, "F7 keys differ"):
            phase.verify_generation_chain(self.root)

    def test_a6_must_directly_bind_the_dispatch_fence_correction(self) -> None:
        self.write_chain()
        a7_path = self.root / phase.A7_PATH
        a6 = json.loads(a7_path.read_text())
        del a6[phase.P4_FIELD]
        a7_path.write_bytes(canonical(a6))

        with self.assertRaisesRegex(phase.SuccessorProduceV5Error, "A7 keys differ"):
            phase.verify_generation_chain(self.root)
    ...
    def test_each_future_record_requires_the_exact_p3_identity_shape(self) -> None:
        for label, relative in (
            ...
            with self.subTest(label=label):
                ...
                document[phase.P4_FIELD]["extra"] = False
    ...
    def test_dispatch_claim_rejects_git_ref_liveness_traps(self) -> None:
        for attempt_id in ("a..b", "a.lock", "a."):
            with self.subTest(attempt_id=attempt_id):
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "Git tag"
                ):
    ...
    def test_dispatch_claim_rejects_message_or_runtime_context_drift(self) -> None:
        ...
        mutations = (
            (raw + b"\n", run_id, "1", phase.V5_WORKFLOW_PATH, head_sha, head_authority_sha256),
            (raw, "33299900002", "1", phase.V5_WORKFLOW_PATH, head_sha, head_authority_sha256),
            (raw, run_id, "2", phase.V5_WORKFLOW_PATH, head_sha, head_authority_sha256),
            (raw, run_id, "1", ".github/workflows/other.yml", head_sha, head_authority_sha256),
            (raw, run_id, "1", phase.V5_WORKFLOW_PATH, "c" * 40, head_authority_sha256),
            (raw, run_id, "1", phase.V5_WORKFLOW_PATH, head_sha, "d" * 64),
        )
        for message, observed_run, attempt, workflow, observed_head, observed_a6 in mutations:
            with self.subTest(run=observed_run, attempt=attempt, workflow=workflow):
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "dispatch claim"
                ):
                    phase.verify_dispatch_claim(
                        chain,
                        claim_ref=phase.dispatch_claim_ref(chain.attempt_id),
                        ...
                        head_authority_sha256=observed_a6,
                    )
        ...
        with mock.patch.object(phase.sys, "stdout", output):
            code = phase.main(
                [
                    ...
                    "1",
                    "--event-name",
                    phase.EXACT_EVENT_NAME,
                    "--dispatch-ref",
                    phase.EXACT_DISPATCH_REF,
                    "--workflow-path",
                    phase.V5_WORKFLOW_PATH,
                    "--workflow-ref",
                    phase.EXACT_WORKFLOW_REF,
                    "--head-sha",
                    head_sha,
    ...
    def test_dispatch_claim_verifier_binds_ref_tag_object_and_target_commit(self) -> None:
        ...
        run_id = dispatch["github_run_id"]
        head_sha = dispatch["head_sha"]
        head_authority_sha256 = dispatch["head_authority_sha256"]
    ...
    def test_production_requires_dispatch_capability_before_backend_prepare(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "dispatch capability"
        ):
            phase.produce(
                repository_root=self.root,
                artifact_store=store,
    ...
    def test_production_rechecks_dispatch_capability_immediately_before_marker(self) -> None:
        ...
        def expire_before_marker(*args, **kwargs):
            ...
            verifications += 1
            if verifications == 2:
                raise phase.SuccessorProduceV5Error(
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "expired before marker"
            ):
                phase.produce(
                    repository_root=self.root,
                    artifact_store=store,
    ...
    def test_readback_pass_is_bound_to_current_root_disk_identity(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "readback-v3 image identity differs"
        ):
            phase.qualify(
                repository_root=self.root,
                outputs=outputs,
    ...
    def test_readback_rejects_empty_verification_check_set(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "verification check count differs"
        ):
            phase.qualify(
                repository_root=self.root,
                outputs=outputs,
    ...
    def test_readback_rejects_entry_count_not_derived_from_sealed_staging(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "readback-v3 entry count differs"
        ):
            phase.qualify(
                repository_root=self.root,
                outputs=outputs,
    ...
    def test_readback_verification_requires_exact_seven_rows(self) -> None:
        ...
        for label, mutate in mutations.items():
            with self.subTest(label=label):
                ...
                with self.assertRaises(phase.SuccessorProduceV5Error):
    ...
    def test_output_pin_rejects_two_image_names_for_one_inode(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "distinct inodes"
        ):
    ...
    def test_pinned_metadata_rejects_name_replacement_before_parse(self) -> None:
        ...
        with phase._pinned_outputs(output_root, (name,)) as pinned:
            ...
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "changed while it was pinned"
            ):
    ...
    def test_read_regular_byte_limit_survives_growth_after_open(self) -> None:
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "metadata exceeds byte limit"
            ):
                phase._read_regular(
    ...
    def test_read_regular_rejects_same_size_timestamp_mutation_while_reading(
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "changed while it was read"
            ):
    ...
    def test_marker_rejects_integer_alias_for_consumed_boolean(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "consumed marker differs from A7"
        ):
            phase.qualify(
                repository_root=self.root,
                outputs=outputs,
    ...
    def test_qualification_rejects_pending_changed_during_readback(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "qualification output identity changed"
        ):
            phase.qualify(
                repository_root=self.root,
                outputs=outputs,
    ...
    def test_existing_qualified_result_is_strictly_reverified(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "qualified result differs"
        ):
            phase.qualify(
                repository_root=self.root,
                outputs=outputs,
    ...
    def test_replica_comparison_requires_two_exact_qualified_directories(self) -> None:
        ...
        self.assertEqual(result["attemptId"], "mac3-launcher-v2-successor-v5-attempt-1")
        self.assertEqual(
    ...
    def test_replica_comparison_rejects_extra_missing_or_unqualified_members(self) -> None:
        ...
        for label, mutate in mutations.items():
            with self.subTest(label=label):
                ...
                mutate(right)
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "qualified replica members differ"
                ):
    ...
    def test_replica_comparison_rejects_byte_drift_and_directory_reuse(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "qualified replica contents differ"
        ):
            phase.compare_qualified_replicas(
                repository_root=self.root,
                left=left,
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "distinct directories"
        ):
            phase.compare_qualified_replicas(
                repository_root=self.root,
                left=left,
    ...
    def test_replica_comparison_rejects_matching_false_embedded_claims(self) -> None:
        ...
        for label, mutate, message in (
            ...
            with self.subTest(label=label):
                ...
                with self.assertRaisesRegex(phase.SuccessorProduceV5Error, message):
                    phase.compare_qualified_replicas(
                        repository_root=self.root,
                        left=left,
                        right=right,
    ...
    def test_provenanced_comparison_requires_distinct_matrix_replica_envelopes(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "replica provenance differs"
        ):
    ...
    @staticmethod
    def _dispatch_context_cli_arguments(dispatch: dict[str, object]) -> list[str]:
        return [
            "--github-run-id",
            ...
            "--event-name",
            str(dispatch["event_name"]),
            "--dispatch-ref",
            str(dispatch["dispatch_ref"]),
            "--workflow-path",
            str(dispatch["workflow_path"]),
            "--workflow-ref",
            str(dispatch["workflow_ref"]),
            "--head-sha",
            str(dispatch["head_sha"]),
            "--head-authority-sha256",
            str(dispatch["head_authority_sha256"]),
        ]

    @classmethod
    def _dispatch_cli_arguments(cls, dispatch: dict[str, object]) -> list[str]:
        return [
            "--claim-ref",
            ...
            *cls._dispatch_context_cli_arguments(dispatch),
        ]
    ...
    def test_failure_bundle_seal_accepts_only_the_exact_progress_states(self) -> None:
        ...
        for index, names in enumerate(invalid):
            with self.subTest(invalid=sorted(names)):
                ...
                parent_info = parent.stat()
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "bundle|member set"
                ):
                    phase.seal_collectable_replica_bundle(
                        parent=parent,
                        successful=False,
    ...
    def test_output_recovery_converges_marker_crash_states_and_binds_claim(
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "marker|claim|attempt|A7|output"
        ):
            phase.reconcile_production_output_state(
                repository_root=self.root,
                parent=wrong_parent,
    ...
    def test_failure_seal_requires_and_finally_rechecks_the_exact_marker(
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "marker|claim|required"
        ):
            phase.seal_collectable_replica_bundle(
        ...
        wrong = dict(expected)
        wrong["attemptId"] = "mac3-launcher-v2-successor-v5-attempt-2"
        ...
        try:
            with mock.patch.object(
                ...
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "marker|claim|changed|content"
                ):
                    phase.seal_collectable_replica_bundle(
                        parent=race_parent,
                        successful=False,
    ...
    def test_failure_seal_rejects_unverified_provenance_without_mutation(
        ...
        parent_info = parent.stat()
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "provenance|parent members|bundle"
        ):
            phase.seal_collectable_replica_bundle(
                parent=parent,
                successful=False,
    ...
    def test_output_recovery_rejects_unsafe_sealed_members_and_rebinds_parent(
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "sealed|mode|member|bundle|output"
        ):
            phase.reconcile_production_output_state(
                repository_root=self.root,
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "provenance|sealed|symlink|member"
        ):
            phase.reconcile_production_output_state(
                repository_root=self.root,
        ...
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "parent|path|identity|changed"
            ):
                phase.reconcile_production_output_state(
                    repository_root=self.root,
                    parent=race_parent,
    ...
    def test_success_bundle_seal_rejects_missing_or_changed_provenance(self) -> None:
        ...
        parent_info = parent.stat()
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "bundle|provenance"
        ):
            phase.seal_collectable_replica_bundle(
                parent=parent,
                successful=True,
    ...
    def test_integrated_seal_rejects_wrong_parent_before_provenance_or_mode_effects(
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "parent identity"
        ):
    ...
    def test_integrated_seal_rejects_same_name_output_inode_replacement(self) -> None:
        ...
        with mock.patch.object(phase.os, "fchmod", side_effect=replace_on_first_file):
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "changed|identity|seal"
            ):
                self.publish_and_seal_fixture(parent, outputs, dispatch)
    ...
    def test_integrated_seal_rejects_in_place_content_mutation_during_seal(
        ...
        try:
            with mock.patch.object(phase.os, "fchmod", side_effect=mutate_after_seal):
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "changed|content|seal"
                ):
                    self.publish_and_seal_fixture(parent, outputs, dispatch)
        finally:
            os.close(retained)
    ...
    def test_recovery_record_is_create_once_and_binds_mount_scratch_and_parent(
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "create-once|already exists"
        ):
    ...
    def test_completed_recovery_removal_uses_a_reentrant_tombstone(self) -> None:
        ...
        with mock.patch.object(phase.os, "unlink", side_effect=fail_after_tombstone):
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "cleanup|recovery|remove"
            ):
    ...
    def test_recovery_record_rejects_mount_or_parent_drift_without_rewriting(self) -> None:
        ...
        drifted["mountId"] = "201"
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "mount|recovery"
        ):
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "parent|recovery"
        ):
            phase.verify_production_recovery_record(
                scratch=scratch,
                outputs_parent=outputs_parent,
    ...
    def test_recovery_record_rejects_a_plain_directory_claimed_as_tmpfs(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "live|mount|tmpfs|recovery"
        ):
            phase.publish_production_recovery_record(
                scratch=scratch,
                expected_scratch_identity=(scratch_info.st_dev, scratch_info.st_ino),
    ...
    def test_recovery_record_requires_empty_staging_and_a_separate_output_tree(
        ...
        for ordinal, nested_output in enumerate((False, True), start=1):
            with self.subTest(nested_output=nested_output):
                ...
                staging_info = staging.stat()
                parent_info = outputs_parent.stat()
                stem = "boole-nsv5-" + f"{ordinal + 3}" * 40 + "-r2"
                mount_identity = {
                    "fileSystemType": "tmpfs",
                    "majorMinor": (
    ...
    def test_recovery_record_rejects_a_symlinked_ancestor(self) -> None:
        ...
        with mock.patch.object(
            ...
            phase.SuccessorProduceV5Error, "symlink|path|directory|recovery"
        ):
            phase.publish_production_recovery_record(
                scratch=scratch,
                expected_scratch_identity=(scratch_info.st_dev, scratch_info.st_ino),
    ...
    def test_recovery_record_rejects_unexpected_scratch_members(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "member|scratch|recovery"
        ):
            phase.publish_production_recovery_record(
                scratch=scratch,
                expected_scratch_identity=(scratch_info.st_dev, scratch_info.st_ino),
    ...
    def test_recovery_record_verification_rejects_mode_drift(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "mode|recovery"
        ):
            phase.verify_production_recovery_record(
                scratch=scratch,
                outputs_parent=outputs_parent,
    ...
    def test_recovery_record_requires_an_empty_dedicated_output_parent(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "output parent|member|dedicated"
        ):
            phase.publish_production_recovery_record(
                scratch=scratch,
                expected_scratch_identity=(scratch_info.st_dev, scratch_info.st_ino),
    ...
    def test_recovery_record_verification_rejects_hardlink_or_staging_replacement(
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "identity|link|recovery"
        ):
            phase.verify_production_recovery_record(
                scratch=scratch,
                outputs_parent=outputs_parent,
        ...
        staging.mkdir(mode=0o700)
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "staging|mount|recovery"
        ):
            phase.verify_production_recovery_record(
                scratch=scratch,
                outputs_parent=outputs_parent,
    ...
    def test_recovery_record_rechecks_live_directory_names_after_read(self) -> None:
        ...
        for ordinal, replaced_name in enumerate(
            ...
            with self.subTest(replaced_name=replaced_name):
                ...
                staging_info = staging.stat()
                parent_info = outputs_parent.stat()
                stem = "boole-nsv5-" + f"{ordinal}" * 40 + "-r1"
                mount_identity = {
                    "fileSystemType": "tmpfs",
                    "majorMinor": (
                        f"{os.major(staging_info.st_dev)}:"
    ...
    def test_recovery_record_cli_round_trip_is_bounded_and_uses_process_owner(
        ...
        shared = [
            "--repository-root",
            str(self.root),
            *self._dispatch_context_cli_arguments(dispatch),
            "--scratch",
            str(scratch),
            "--outputs-parent",
            str(outputs_parent),
    ...
    def test_post_unmount_requires_zero_exact_mountinfo_matches(self) -> None:
        target = pathlib.Path("/run/boole/native-shadow-successor-v5/staging")
        other = pathlib.Path("/run/boole/native-shadow-successor-v5/other")

        def record(mount_point: pathlib.Path) -> bytes:
            return (
                f"91 22 0:55 / {mount_point} rw,nosuid,nodev - "
                "tmpfs boole-nsv5-fixture rw,size=4096,nr_inodes=16\n"
            ).encode()
    ...
    def test_recovery_mount_identity_requires_the_exact_tmpfs_caps(self) -> None:
        staging = pathlib.Path("/run/boole/native-shadow-successor-v5/staging")
        stem = "boole-nsv5-" + "a" * 40 + "-r1"
        base = {
            "fileSystemType": "tmpfs",
            "majorMinor": "0:55",
    ...
    def test_controlled_import_rejects_unbound_transitive_before_side_effect(self) -> None:
        ...
        try:
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "unbound repository import"
            ):
    ...
    def test_isolated_python_opens_repo_imports_only_after_binding_verification(self) -> None:
        source = (
            ...
            "g=runpy.run_path(str(root/'scripts/native_shadow_successor_produce_phase_arm64_v5.py'));"
            "assert str(root) not in sys.path;"
    ...
    def test_script_entrypoint_owns_the_preregistered_v4_bootstrap_before_r2(
        ...
        source = (
            ...
            f"root=pathlib.Path({str(REPOSITORY_ROOT)!r});"
            "path=root/'scripts/native_shadow_successor_produce_phase_arm64_v5.py';"
    ...
    def test_controlled_import_rejects_preloaded_bound_module(self) -> None:
        ...
        try:
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "already loaded"
            ):
    ...
    def test_controlled_import_rejects_tampered_bound_module_before_loader(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "bound repository module identity differs"
        ):
    ...
    def test_controlled_import_rejects_module_that_changes_its_own_bytes(self) -> None:
        ...
        try:
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "post-import repository module identity differs"
            ):
                phase.RepositoryImageBackend()._controlled_imports(request, (name,))
        finally:
            sys.modules.pop(name, None)
    ...
    def test_fresh_rehearsal_can_seal_r2_without_future_authority_or_outputs(self) -> None:
        ...
        with mock.patch.object(
            ...
            result = phase.rehearse(
                ...
                launcher=launcher,
                backend=backend,
                expected_systemd_unit="boole-nsv5-rehearsal-ABC123.service",
            )
        ...
        self.assertEqual(result["schema"], phase.R3_SCHEMA)
        self.assertEqual(result["status"], phase.R3_STATUS)
            ...
            [
                mock.call(
                    expected_systemd_unit=(
                        "boole-nsv5-rehearsal-ABC123.service"
                    )
                ),
                mock.call(
                    expected_systemd_unit=(
        ...
        self.assertEqual(
            result[phase.P4_FIELD],
            identity(self.root, P4_RELATIVE),
        )
        self.assertFalse((self.root / phase.R3_PATH).exists())
    ...
    def test_a6_integer_budget_rejects_boolean_alias(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "A7 dispatch count differs"
        ):
            phase.verify_generation_chain(self.root)
    ...
    def test_a6_boolean_authority_rejects_integer_alias(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "A7 grants more or less"
        ):
            phase.verify_generation_chain(self.root)
    ...
    def test_r2_integer_effect_rejects_boolean_alias(self) -> None:
        ...
        f7_path = self.root / phase.F7_PATH
        f6 = json.loads(f7_path.read_text())
        f6["predecessors"][-1] = r3_identity
        f7_path.write_bytes(canonical(f6))
        a7_path = self.root / phase.A7_PATH
        a6 = json.loads(a7_path.read_text())
        a6["predecessors"][-2:] = [r3_identity, identity(self.root, phase.F7_PATH)]
        a7_path.write_bytes(canonical(a6))

        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "R3 effect accounting differs"
        ):
            phase.verify_generation_chain(self.root)
    ...
    def test_f6_false_boundary_rejects_integer_alias(self) -> None:
        ...
        a7_path = self.root / phase.A7_PATH
        a6 = json.loads(a7_path.read_text())
        a6["predecessors"][-1] = identity(self.root, phase.F7_PATH)
        a7_path.write_bytes(canonical(a6))

        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "F7 boundaries differs"
        ):
            phase.verify_generation_chain(self.root)

    def test_dangling_withdrawn_reservation_is_not_treated_as_absent(self) -> None:
        self.write_chain()
        p4 = json.loads((self.root / phase.P4_PATH).read_text())
        withdrawn = self.root / p4["unusedReservations"][0]["path"]
        withdrawn.symlink_to("does-not-exist")

        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error,
            "P4 unused reservation 1 must remain absent",
        ):
            phase.verify_generation_chain(self.root)
    ...
    def test_bound_file_swap_between_lstat_and_open_is_rejected(self) -> None:
        ...
        with mock.patch.object(phase.os, "open", side_effect=swapped_open):
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "changed between inspection and open"
            ):
    ...
    def test_bound_file_rejects_symlinked_parent_component(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "parent component"
        ):
    ...
    def test_repository_backend_requires_the_exact_sealed_launcher(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "launcher-v2 sealed identity differs"
        ):
    ...
    def test_layer_rejects_absolute_symlink_with_parent_components(self) -> None:
        ...
        destination = self.root / "tree"
        with self.assertRaisesRegex(phase.SuccessorProduceV5Error, "link escapes"):
    ...
    def test_layer_rejects_guest_root_absolute_symlink_cycle(self) -> None:
        ...
        destination = self.root / "tree"
        with self.assertRaisesRegex(phase.SuccessorProduceV5Error, "symlink cycle"):
            phase.RepositoryImageBackend._extract_layer(raw.getvalue(), destination)
        self.assertFalse(os.path.lexists(destination))
    ...
    def test_layer_rejects_regular_member_below_symlink_parent(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "traverses a symlink parent"
        ):
            phase.RepositoryImageBackend._extract_layer(
                raw.getvalue(), self.root / "tree"
            )
    ...
    def test_layer_rejects_special_member(self) -> None:
        for index, member_type in enumerate(
            ...
            with self.subTest(member_type=member_type):
                ...
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "contains special member"
                ):
                    phase.RepositoryImageBackend._extract_layer(
    ...
    def test_layer_rejects_hardlink_through_symlink(self) -> None:
        ...
        with self.assertRaisesRegex(phase.SuccessorProduceV5Error, "hardlink"):
            phase.RepositoryImageBackend._extract_layer(
                raw.getvalue(), self.root / "tree"
            )
    ...
    def test_output_parent_fsync_failure_precedes_marker_and_image_effects(self) -> None:
        ...
        def fail_parent(directory):
            if pathlib.Path(directory).resolve() == outputs.parent.resolve():
                raise phase.SuccessorProduceV5Error("injected output-parent fsync failure")
            return real_fsync(directory)

        backend = Backend()
        with mock.patch.object(phase, "_fsync_directory", side_effect=fail_parent):
            with self.assertRaisesRegex(
                phase.SuccessorProduceV5Error, "output-parent fsync failure"
            ):
                phase.produce(
                    repository_root=self.root,
                    artifact_store=store,
    ...
    def test_collectability_failure_preserves_original_failure(self) -> None:
        ...
        with mock.patch.object(
            ...
            with self.assertRaises(phase.SuccessorProduceV5Error) as raised:
                phase.produce(
                    repository_root=self.root,
                    artifact_store=store,
                    outputs=outputs,
    ...
    def test_collectability_rejects_external_hardlinks_without_changing_them(self) -> None:
        ...
        with self.assertRaisesRegex(
            phase.SuccessorProduceV5Error, "hardlink|link count"
        ):
    ...
    def test_collectability_rejects_special_or_executable_members(self) -> None:
        for unsafe_mode in (0o1644, 0o755, 0o666, 0o620):
            with self.subTest(mode=oct(unsafe_mode)):
                ...
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "unsafe mode"
                ):
                    phase._make_outputs_collectable(outputs)
    ...
    def test_collectability_rejects_unknown_or_nested_output_members(self) -> None:
        for member_name, is_directory in (("unknown", False), ("nested", True)):
            with self.subTest(member=member_name):
                ...
                with self.assertRaisesRegex(
                    phase.SuccessorProduceV5Error, "member set|unsafe file kind"
                ):
                    phase._make_outputs_collectable(outputs)
