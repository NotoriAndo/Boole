...
PRODUCER="$ROOT/scripts/native_shadow_successor_produce_phase_arm64_v4.py"
readonly v4_workflow_path=".github/workflows/native-shadow-successor-produce-arm64-v4.yml"
readonly a6_relative="native/containment/native-shadow-mac3-successor-production-authority-arm64-v6.json"
readonly claim_ref_prefix="refs/tags/boole-native-shadow-mac3-successor-production-a6-"
...
readonly root_anchor_prefix="/var/lib/boole/native-shadow-successor-v4/anchors"
readonly root_export_prefix="/var/lib/boole/native-shadow-successor-v4/exports"
...
# The v4 production authority seals a 2 GiB / 200,000-entry assembled-tree
...
# sealed tree of process/tool overhead.  R2 must exercise these exact finite
...
# closed-local production took about six minutes end to end, but R2 must still
...
die() {
  printf 'native-shadow launcher-v2 successor v4: FAIL: %s\n' "$*" >&2
  exit 1
}
...
require_root_execution_anchor() {
  ...
  case $mode in
    produce|cleanup-only)
      ...
      ;;
    rehearsal|preflight)
      [[ $ROOT =~ ^/var/lib/boole/native-shadow-successor-v4/anchors/[0-9a-f]{40}-(rehearsal|preflight)/repo$ ]] \
  ...
  for protected_file in \
    "$ROOT/scripts/native-shadow-successor-produce-arm64-v4.sh" \
    "$ROOT/scripts/native_shadow_successor_produce_phase_arm64_v4.py"; do
...
# P2 preregisters these v4 bytes and P3 corrects their future dispatch fence.
# Verify both and P2's live predecessors with a stdlib-only process before
# importing the v4 producer.  R2/F6/A6 are deliberately not required by this
# first gate: R2 is what the rehearsal of these exact bytes will create
# evidence for.
...
if not isinstance(authorisations, dict) or any(value not in (False, 0) for value in authorisations.values()):
    raise SystemExit("P2 grants an authority")
runs = document.get("runs")
if not isinstance(runs, dict) or any(type(value) is not int or value != 0 for value in runs.values()):
    raise SystemExit("P2 run accounting is not zero")
bindings = document.get("bindings")
if not isinstance(bindings, list) or len(bindings) != 2:
    raise SystemExit("P2 direct bindings differ")
for row in bindings:
    if not isinstance(row, dict) or set(row) != {"path", "role", "sha256", "sizeBytes"}:
        raise SystemExit("P2 binding shape differs")
    regular(row["path"], row["sizeBytes"], row["sha256"])
unused = document.get("unusedReservedPaths")
if not isinstance(unused, list) or len(unused) != 2:
    raise SystemExit("P2 withdrawn reservations differ")
for row in unused:
    if row.get("requiredAbsent") is not True or row.get("reuseForbidden") is not True:
        raise SystemExit("P2 withdrawn reservation policy differs")
    if os.path.lexists(root / row["path"]):
        raise SystemExit("withdrawn v5 reservation exists: " + row["path"])
...
try:
    ...
    raise SystemExit("P3 dispatch-fence correction is invalid: " + str(exc))
try:
    p3 = json.loads(p3_raw.decode("utf-8"))
except (UnicodeDecodeError, json.JSONDecodeError) as exc:
    raise SystemExit("P3 dispatch-fence correction is not JSON: " + str(exc))
if p3_raw != (json.dumps(p3, indent=2, sort_keys=True) + "\n").encode():
    raise SystemExit("P3 dispatch-fence correction is not canonical JSON")
if p3.get("schema") != (
    "boole.native-shadow.mac3.launcher-v2-successor-production-dispatch-"
    "fence-correction.arm64.v1"
):
    raise SystemExit("P3 schema differs")
if p3.get("status") != (
    "CORRECTED-BEFORE-R2-NO-PRODUCTION-DISPATCH-AUTHORITY"
):
    raise SystemExit("P3 status differs")
expected_p3_authorisations = {
...
if p3.get("authorisations") != expected_p3_authorisations:
    raise SystemExit("P3 grants an authority")
if p3.get("runs") != expected_p3_runs:
    raise SystemExit("P3 run accounting is not zero")
claim = p3.get("claimFence", {}).get("repositoryClaim", {})
if claim.get("attemptIdPattern") != "^[a-z0-9][a-z0-9._-]{0,127}$":
    raise SystemExit("P3 attempt ID pattern differs")
future = p3.get("futureBindingRequirement", {})
if future.get("fieldName") != "productionDispatchFenceCorrection":
    raise SystemExit("P3 future binding field differs")
if future.get("fieldKeys") != ["path", "sha256", "sizeBytes"]:
    raise SystemExit("P3 future binding identity differs")
if future.get("correctionPath") != p3_relative:
    raise SystemExit("P3 future binding path differs")
if future.get("exactKeysOnly") is not True or future.get(
    ...
    raise SystemExit("P3 future direct-binding policy differs")
' "$ROOT"
}
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --github-repository) [[ $# -ge 2 ]] || die "--github-repository needs a value"; github_repository=$2; shift 2 ;;
    --github-workflow-ref) [[ $# -ge 2 ]] || die "--github-workflow-ref needs a value"; github_workflow_ref=$2; shift 2 ;;
...
if [[ $mode != "dispatch-claim-message" && $mode != "dispatch-claim-verify" \
      ...
      && ( -n $github_run_id || -n $github_run_attempt \
           || -n $github_repository || -n $github_workflow_ref \
           || -n $head_sha || -n $claim_ref \
...
if [[ $mode == "produce" ]]; then
  for value_name in github_run_id github_run_attempt github_repository \
    github_workflow_ref head_sha claim_ref expected_tag_object_sha \
...
if [[ $mode == "verify-bindings" ]]; then
  printf 'native-shadow launcher-v2 successor v4: bindings verified\n' >&2
  exit 0
fi
...
prepare_dispatch_context() {
  ...
  local head_a6_line=""
  local attempt_id=""
  local workflow_prefix=""

  for value_name in github_run_id github_run_attempt github_repository \
    github_workflow_ref head_sha; do
    [[ -n ${!value_name} ]] || die "the dispatch claim lacks $value_name"
  done
  [[ $github_run_attempt == "1" ]] \
    || die "the dispatch claim is only valid for github.run_attempt 1"
  workflow_prefix="$github_repository/$v4_workflow_path@"
  [[ $github_workflow_ref == "$workflow_prefix"* ]] \
    || die "the live workflow path differs"
  [[ ${#github_workflow_ref} -gt ${#workflow_prefix} ]] \
    || die "the live workflow ref has no Git ref"
  ...
  head_a6_line="$(git_repo cat-file blob "$head_sha:$a6_relative" | sha256sum)" \
    || die "the HEAD A6 blob cannot be hashed"
  head_a6_sha256="${head_a6_line%% *}"
  [[ $head_a6_line == "$head_a6_sha256  -" ]] \
    || die "the HEAD A6 digest output differs"

  python3 -I -S "$PRODUCER" production-check --repository-root "$ROOT" \
    >/dev/null
  attempt_id="$(git_repo cat-file blob "$head_sha:$a6_relative" | python3 -I -S -c '
import json
import sys
raw = sys.stdin.buffer.read(1048577)
if len(raw) > 1048576:
    raise SystemExit("A6 exceeds the metadata byte limit")
document = json.loads(raw.decode("utf-8"))
if raw != (json.dumps(document, indent=2, sort_keys=True) + "\n").encode():
    raise SystemExit("A6 is not canonical JSON")
grant = document.get("grant")
if not isinstance(grant, dict):
    raise SystemExit("A6 grant differs")
attempt_id = grant.get("attemptId")
if not isinstance(attempt_id, str):
    raise SystemExit("A6 attempt ID differs")
sys.stdout.write(attempt_id)
')" || die "the HEAD A6 attempt ID cannot be read"
...
verify_live_dispatch_claim() {
  ...
  git_repo cat-file tag "$tag_object_sha" | \
    python3 -I -S "$PRODUCER" dispatch-claim-verify \
      ...
      --workflow-path "$v4_workflow_path" \
      --head-sha "$head_sha" \
      --head-a6-sha256 "$head_a6_sha256" \
      >/dev/null
  recheck_dispatch_claim_ref
}
...
if [[ $mode == "production-check" ]]; then
  for required in \
    ...
    native/containment/native-shadow-mac3-successor-production-authority-arm64-v6.json; do
...
require_host() {
  ...
  [[ $(uname -s) == "Linux" ]] || die "the v4 producer requires Linux"
  [[ $(uname -m) == "aarch64" || $(uname -m) == "arm64" ]] \
    || die "the v4 producer requires native arm64"
}
...
require_cleanup_host() {
  ...
  [[ $(uname -s) == "Linux" ]] || die "the v4 cleanup requires Linux"
  [[ $(uname -m) == "aarch64" || $(uname -m) == "arm64" ]] \
    || die "the v4 cleanup requires native arm64"
}
...
initialise_production_recovery_identity() {
  ...
  recovery_stem="boole-nsv4-${expected_tag_object_sha}-r${replica_ordinal}"
  production_scratch="/run/boole/native-shadow-successor-v4/${recovery_stem}"
  recovery_lock_path="/run/boole/native-shadow-successor-v4/${recovery_stem}.lock"
...
require_root_recovery_parent() {
  ...
  for candidate in /run/boole /run/boole/native-shadow-successor-v4; do
...
acquire_production_recovery_lock() {
  ...
  local lock_parent="/run/boole/native-shadow-successor-v4"
...
resume_successful_parent() {
  ...
  resume_artifact_name="native-shadow-successor-v4-replica-${replica_ordinal}"
  recheck_dispatch_claim_ref \
  ...
  if ! git_repo cat-file tag "$tag_object_sha" | \
    /usr/bin/env -i PATH="$PATH" HOME=/nonexistent LANG=C LC_ALL=C \
      ...
      /usr/bin/python3 -I -S "$PRODUCER" publish-and-seal-replica \
        ...
        --workflow-path "$v4_workflow_path" \
        --head-sha "$head_sha" \
        --head-a6-sha256 "$head_a6_sha256"; then
...
require_claim_bound_export_parent() {
  local candidate=$1
  local expected="/var/lib/boole/native-shadow-successor-v4/exports/$expected_tag_object_sha/replica-$replica_ordinal"
  local ancestor=""
  local mode=""
  [[ $expected_tag_object_sha =~ ^[0-9a-f]{40}$ ]] \
...
recover_production_state() {
  ...
  if [[ ! -e $scratch && ! -L $scratch ]]; then
    ...
    printf 'native-shadow launcher-v2 successor v4: cleanup already-clean\n' >&2
    return 0
  fi
  [[ -d $scratch && ! -L $scratch ]] \
...
if [[ $mode == "rehearsal" ]]; then
  ...
  rehearsal_root="$(mktemp -d /tmp/boole-native-shadow-successor-v4-rehearsal.XXXXXX)"
  expected_scratch_prefix="/tmp/boole-native-shadow-successor-v4-rehearsal."
  rehearsal_unit="boole-nsv4-rehearsal-${rehearsal_root##*.}.service"
  ...
  cleanup() {
    ...
    if (( primary_status != 0 )); then
      if (( cleanup_status != 0 )); then
        printf 'native-shadow launcher-v2 successor v4: cleanup also failed after primary status %s\n' \
          "$primary_status" >&2
      fi
      exit "$primary_status"
    fi
...
expected_scratch_prefix="/tmp/boole-native-shadow-successor-v4."
if [[ $mode == "produce" ]]; then
  scratch="$production_scratch"
  require_root_recovery_parent
  acquire_production_recovery_lock
  ...
  scratch="$(mktemp -d /tmp/boole-native-shadow-successor-v4.XXXXXX)"
  preflight_unit="boole-nsv4-preflight-${scratch##*.}.service"
fi
if [[ $mode == "produce" ]]; then
  expected_mount_source="$recovery_stem"
else
...
if [[ $mode == "produce" ]]; then
  ...
  production_argv+=(
    ...
    --outputs "$outputs"
    --result "$outputs/PRODUCE-RESULT-PENDING-READBACK-V4.json"
    --claim-ref "$claim_ref"
    --ref-object-sha "$ref_object_sha"
    --tag-object-sha "$expected_tag_object_sha"
    --github-run-id "$github_run_id"
  ...
  qualification_argv+=(
    ...
    --repository-root "$ROOT"
    --outputs "$outputs"
    --pending "$outputs/PRODUCE-RESULT-PENDING-READBACK-V4.json"
    --result "$result"
  )
