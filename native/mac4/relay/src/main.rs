...
mod linux {
    ...
    use boole_native_shadow_mac4_relay::{
        decode_hello, encode_ready, GuestReady, FRAME_BYTES, HOST_CID, VSOCK_PORT,
    };
    ...
    fn accept_host(listener: &Descriptor) -> io::Result<Descriptor> {
        ...
        set_timeout(&accepted, SO_RCVTIMEO)?;
        set_timeout(&accepted, SO_SNDTIMEO)?;
        Ok(accepted)
    }

    fn set_timeout(descriptor: &Descriptor, option: i32) -> io::Result<()> {
        let timeout = Timeval {
            seconds: 10,
            microseconds: 0,
        };
    ...
    fn serve_connection(connection: &Descriptor) -> io::Result<()> {
    ...
    pub fn run() -> io::Result<()> {
        ...
        loop {
            match accept_host(&listener).and_then(|connection| serve_connection(&connection)) {
